"""v0.62 retains the bounded v0.57 journal without a new evidence stage."""
...
def test_restart_keeps_terminal_or_reserved_subject_permanent(
    ...
    error(
        create(service, facts, idempotency_key="different-retained-v062-key"),
...
def test_v062_cannot_relabel_retained_journal(tmp_path, facts, populated):
    ...
    with sqlite3.connect(journal.database_path) as connection:
        ...
        connection.execute("PRAGMA application_id=62")
    before = journal.database_path.read_bytes()
