"""v0.62 retains the closed v0.57 pair; no successor authority is added."""
...
def test_v062_retains_the_frozen_pair_and_bounds():
    source = " ".join(
        (ROOT / "docs/architecture/v0.62-authority-boundary.md").read_text().split()
    )
...
def test_v062_deferred_markers_are_rejected_even_when_rehashed(
...
def test_v062_cannot_relabel_the_retained_schema(retained_pair_member):
    member, field, sign = retained_pair_member
    raw = member.model_dump(mode="python")
    raw["schema"] = raw["schema"].removesuffix("-v1") + "-v062"
    raw[field] = sign(raw)
    with pytest.raises(ValidationError, match="literal type or value mismatch"):
        type(member).model_validate(raw)


def test_v062_retained_evidence_is_deeply_immutable(retained_pair_member):
