...
_SCHEMA = (
    """CREATE TABLE reservations (
        ...
        runtime_plan_review_id TEXT NOT NULL,
        idem TEXT NOT NULL,
        request TEXT NOT NULL,
        reservation_json TEXT NOT NULL,
        UNIQUE (operator_id, idem),
        UNIQUE (operator_id, candidate_record_id, runtime_plan_review_id))""",
...
class WorkerActivationRuntimeInterfacePrerequisiteStoreError(RuntimeError):
...
class WorkerActivationRuntimeInterfacePrerequisiteStore:
    def __init__(
        self,
        ...
        *,
        ...
        with self._connect() as connection:
            ...
            if actual:
                ...
                connection.execute("PRAGMA application_id=57")
    ...
    def _connect(self) -> Iterator[sqlite3.Connection]:
        ...
        try:
            ...
            raise WorkerActivationRuntimeInterfacePrerequisiteStoreError(code) from None
        finally:
        ...
        try:
            ...
            for row in connection.execute("SELECT * FROM reservations"):
                ...
                if row[:6] != (
                    ...
                    reservation.runtime_plan_review_id,
            ...
            for subject, record_json, audit_json in connection.execute(
                ...
                record = self._decode(
                    c.WorkerActivationRuntimeInterfacePrerequisiteV1, record_json
                )
                audit = self._decode(
    ...
    def _validate_record(record, reservation):
        ...
        if (
            ...
            or record.runtime_plan_review_id != reservation.runtime_plan_review_id
    ...
    def _validate_audit(audit, reservation, record):
        if (
            ...
            or audit.runtime_interface_prerequisite_record_fingerprint
            != (
    ...
    def _resolve(self, connection, operator_id, idem, request):
        ...
        return self._decode(
            c.WorkerActivationRuntimeInterfacePrerequisiteV1, evidence[0]
        )
    ...
    def append(
        ...
        *,
        ...
        try:
            reservation = c.WorkerActivationRuntimeInterfacePrerequisiteSubjectReservationV1.model_validate(
                reservation
            )
        ...
        except WorkerActivationRuntimeInterfacePrerequisiteStoreError:
    ...
    def _audit(reservation, correlation, record):
        raw = {
            ...
            "worker_activation_runtime_interface_prerequisite_recorded": record
            is not None,
    ...
    def get(
        self, *, operator_id, candidate_record_id, runtime_interface_prerequisite_id
    ):
        with self._connect() as connection:
            for (payload,) in connection.execute(
                ...
                record = self._decode(
                    c.WorkerActivationRuntimeInterfacePrerequisiteV1, payload
                )
                if (
    ...
    def list_owned(self, *, operator_id, candidate_record_id):
        with self._connect() as connection:
            return tuple(
                self._decode(c.WorkerActivationRuntimeInterfacePrerequisiteV1, row[0])
