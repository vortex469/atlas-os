"""Explicit Core-local v0.51 queue claim/lease/ack admission service."""
...
class ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionReader(Protocol):
    """Read exact owner-scoped v0.50 prerequisite evidence without effects."""
...
class ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionService:
    """Create/get/list bounded evidence; no queue, worker, Agent, or execution API."""

    def __init__(
        self,
        *,
        prerequisite_reader: ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionReader,
    ...
    def create(
        ...
        *,
        ...
        try:
            ...
            exact_create = ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionCreateV1.model_validate(
        ...
        try:
            existing = self._store.resolve_idempotency(
                ...
                prerequisite_valid_until=exact_create.prerequisite_valid_until,
            )
            if existing is not None:
                return _success(existing, correlation_id, self._server_now())
        except ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionStoreError as error:
        ...
        try:
            prerequisite = self._prerequisite_reader.read_owned(
                operator_id=operator,
                candidate_record_id=candidate_record_id,
                prerequisite_id=exact_create.prerequisite_id,
                prerequisite_valid_until=exact_create.prerequisite_valid_until,
            )
            if prerequisite is None:
                return _failure("not_found", correlation_id)
            prerequisite_record, prerequisite_status = prerequisite
            authority = (
                ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionAuthorityContextV1(
            ...
            if (
                not evaluation.controlled_worker_queue_claim_lease_acknowledgement_admission_recorded
            ):
    ...
    def list(
        ...
        *,
        ...
        ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionCollectionV1
        | tuple[ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionResultV1, ...]
    ):
        if authenticated_operator_id is None:
            return (_failure("unauthenticated", correlation_id),)
        if not permission_verified:
...
def create_controlled_worker_queue_claim_lease_acknowledgement_admission_service(
    *,
    prerequisite_reader: ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionReader,
    store: ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionStore,
    clock: Callable[[], datetime],
    enabled: bool = False,
) -> ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionService:
    """Explicit P2 construction; no production composition calls this."""
    return ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionService(
...
def _validation_failure(
    ...
    if "not active" in lowered and "v0.50" in lowered:
        return _failure("v050_prerequisite_not_active", correlation_id)
    if "not frozen" in lowered and "v0.50" in lowered:
        return _failure("v050_prerequisite_not_frozen", correlation_id)
...
__all__ = (
    "ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionReader",
    "ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionService",
