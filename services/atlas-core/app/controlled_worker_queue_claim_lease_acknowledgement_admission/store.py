"""Durable append-only store for v0.51 queue claim/lease/ack admission."""
...
class ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionStoreError(RuntimeError):
...
class ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionStore:
    ...
    def _initialize(self) -> None:
        ...
        try:
            with self._connect() as connection:
                ...
                connection.execute(
                    """
                    CREATE TABLE IF NOT EXISTS cwqcla_admission_reservations (
                        ...
                        prerequisite_valid_until TEXT NOT NULL,
                ...
                connection.execute(
                    """
                    CREATE TABLE IF NOT EXISTS cwqcla_admission_attempts (
                        operator_id TEXT NOT NULL,
                        admission_id TEXT NOT NULL,
                        ...
                        FOREIGN KEY (operator_id, admission_id)
                            REFERENCES cwqcla_admission_reservations(
                ...
                connection.execute(
                    """
                    CREATE TABLE IF NOT EXISTS cwqcla_admissions (
                        operator_id TEXT NOT NULL,
                        admission_id TEXT NOT NULL,
                        candidate_record_id TEXT NOT NULL,
                        idempotency_key_fingerprint TEXT NOT NULL,
                        ...
                        prerequisite_status_fingerprint TEXT NOT NULL,
            ...
            raise ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionStoreError(
                "unavailable"
            ) from error
    ...
    def resolve_idempotency(
        ...
        *,
        ...
        prerequisite_valid_until: str,
    ) -> ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionV1 | None:
        try:
            with self._connect() as connection:
                reservation = connection.execute(
                    """SELECT * FROM cwqcla_admission_reservations
                ...
                if (
                    ...
                    or reservation["prerequisite_valid_until"]
                    != prerequisite_valid_until
                ):
                    raise ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionStoreError(
                        "idempotency_conflict"
                    )
                row = connection.execute(
                    """SELECT * FROM cwqcla_admissions
                    WHERE operator_id = ? AND admission_id = ?""",
                    (operator_id, reservation["admission_id"]),
                ).fetchone()
        except ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionStoreError:
            raise
        except sqlite3.Error as error:
            raise ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionStoreError(
                "unavailable"
    ...
    def append(
        ...
        *,
        ...
        idempotency_reservation: ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionIdempotencyReservationV1,
        subject_reservation: ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionSubjectReservationV1,
        audit_evidence: ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionAuditEvidenceV1,
        prerequisite_valid_until: str,
        force_indeterminate: bool = False,
    ) -> tuple[ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionV1, bool]:
        values = (
        ...
        self._reserve_attempt(
            ...
            prerequisite_valid_until=prerequisite_valid_until,
        )
        ...
        *,
        ...
        try:
            with self._connect() as connection:
                ...
                connection.execute(
                    """INSERT OR IGNORE INTO cwqcla_admission_attempts (
    ...
    ) -> tuple[ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionV1, ...]:
        try:
            with self._connect() as connection:
            ...
            with self._connect() as connection:
                ...
                if rows:
                    ...
                    existing = connection.execute(
                        """SELECT 1 FROM cwqcla_admissions
                        WHERE operator_id = ? AND admission_id = ?""",
                ...
                count = connection.execute(
                    """SELECT COUNT(*) FROM cwqcla_admission_reservations
                    WHERE operator_id = ?""",
                    (record.operator_id,),
                ).fetchone()[0]
                ...
                connection.execute(
                    """INSERT INTO cwqcla_admission_reservations (
                    ...
                    prerequisite_valid_until, reserved_at, idempotency_json,
        ...
        *,
        ...
        try:
            with self._connect() as connection:
                ...
                connection.execute(
                    """INSERT INTO cwqcla_admissions (
                    operator_id, admission_id, candidate_record_id,
                    idempotency_key_fingerprint, request_fingerprint,
                    subject_fingerprint, admission_record_fingerprint,
                    prerequisite_id, prerequisite_record_fingerprint,
                    ...
                    (
                        ...
                        record.prerequisite_status_fingerprint.value,
    ...
    def _is_exact_reservation(
        ...
        return (
            ...
            and row["prerequisite_valid_until"] == prerequisite_valid_until
    ...
    def _validate_reservation(
        ...
        try:
            ...
            idempotency = ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionIdempotencyReservationV1.model_validate_json(
                row["idempotency_json"]
            )
            reservation = ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionSubjectReservationV1.model_validate_json(
    ...
    def _is_row_reservation_exact(
        row: sqlite3.Row,
        idempotency: ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionIdempotencyReservationV1,
        reservation: ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionSubjectReservationV1,
    ) -> bool:
        return (
    ...
    def _decode_record(
        ...
        *,
        ...
        try:
            ...
            audit = ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionAuditEvidenceV1.model_validate_json(
                row["audit_json"]
            )
            if (
                ...
                != record.prerequisite_record_fingerprint.value
                or row["prerequisite_status_fingerprint"]
                != record.prerequisite_status_fingerprint.value
                or reservation_row["prerequisite_valid_until"]
                != record.controlled_worker_queue_claim_lease_acknowledgement_prerequisite.valid_until
