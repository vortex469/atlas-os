...
from app.controlled_worker_queue_claim_lease_acknowledgement_admission.service import (
...
from app.controlled_worker_queue_claim_lease_acknowledgement_admission.test_contract import (
    _facts,
)
...
def _service(
    ...
    *,
    ...
    prerequisite_reader = Reader(
        ...
        else (prerequisite, prerequisite_status)
    )
    admission_store = ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionStore(
...
def _record(admission_service, prerequisite, create, **changes):
    values = {
...
def test_create_get_list_and_restart_safe_owner_readback(tmp_path: Path) -> None:
    admission_service, admission_store, reader, prerequisite, _status, create = (
        _service(tmp_path)
    )
    created = _record(admission_service, prerequisite, create)
    assert created.ok
    assert created.record.admission_state == "recorded"
    assert (
...
def test_default_disabled_auth_permission_missing_and_redaction(
    ...
    assert _record(admission_service, prerequisite, create).error.error_code == (
        "installation_capability_unsupported"
    )
    assert _record(
        admission_service, prerequisite, create, authenticated_operator_id=None
    ).error.error_code == "unauthenticated"
    assert _record(
        admission_service, prerequisite, create, permission_verified=False
    ...
    missing = _service(tmp_path / "missing", prerequisite_evidence=None)
...
def test_exact_duplicate_zero_reader_and_secret_free_persistence(
    ...
    duplicate = _record(duplicate_service, prerequisite, create)
    assert duplicate.ok
    ...
    with sqlite3.connect(admission_store.database_path) as connection:
        rows = []
        for table in (
            "cwqcla_admission_reservations",
            "cwqcla_admissions",
            "cwqcla_admission_attempts",
        ):
            rows.extend(connection.execute(f"SELECT * FROM {table}").fetchall())
        schema_rows = connection.execute(
            "SELECT sql FROM sqlite_master WHERE name LIKE 'cwqcla_admission%'"
        ).fetchall()
...
def test_idempotency_subject_conflicts_and_prerequisite_validation(
    ...
    assert _record(admission_service, prerequisite, create).ok
    ...
    changed = create.model_copy(
        update={"prerequisite_valid_until": "2026-08-27T12:00:44Z"}
    )
    assert _record(admission_service, prerequisite, changed).error.error_code == (
...
def test_concurrent_duplicate_reservation_yields_one_durable_record(
    ...
    admission_service, admission_store, _reader, prerequisite, _status, create = (
        _service(tmp_path)
    )
    with ThreadPoolExecutor(max_workers=8) as pool:
        results = list(
            pool.map(
                lambda _index: _record(admission_service, prerequisite, create),
                range(8),
            )
        )
...
def test_indeterminate_reservation_is_terminal_across_restart(tmp_path: Path) -> None:
    _service_obj, admission_store, _reader, prerequisite, prerequisite_status, create = (
        _service(tmp_path)
    )
    ...
    audit = build_audit(
        record,
        event="controlled_worker_queue_claim_lease_acknowledgement_admission_indeterminate",
    ...
    try:
        admission_store.append(
            ...
            prerequisite_valid_until=create.prerequisite_valid_until,
            force_indeterminate=True,
        )
    except store.ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionStoreError as error:
        assert error.code == "append_indeterminate"
    retry = create_controlled_worker_queue_claim_lease_acknowledgement_admission_service(
        prerequisite_reader=Reader((prerequisite, prerequisite_status)),
        store=ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionStore(
            admission_store.database_path
        ),
        clock=_clock(),
    ...
    result = _record(retry, prerequisite, create)
...
def test_quota_bounds_corruption_and_append_only_surface(tmp_path: Path) -> None:
    ...
    assert not any(
        hasattr(ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionStore, name)
        for name in (
            "delete",
            "release",
            "activate",
    ...
    assert "DELETE FROM cwqcla_admission" not in source
    assert "UPDATE cwqcla_admission" not in source
    ...
    with sqlite3.connect(corrupt[1].database_path) as connection:
        connection.execute("UPDATE cwqcla_admissions SET record_json = ?", ("{}",))
    ...
    with sqlite3.connect(lineage[1].database_path) as connection:
        connection.execute(
            "UPDATE cwqcla_admission_reservations SET prerequisite_valid_until = ?",
...
def test_service_store_have_no_effect_imports_calls_or_production_consumers() -> None:
    ...
    for path in root.rglob("*.py"):
        ...
        if (
            "controlled_worker_queue_claim_lease_acknowledgement_admission.service"
            in source_text
            or "ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionService"
            in source_text
        ):
...
def test_agent_and_execution_worker_do_not_consume_v051_admissions() -> None:
