...
_SCHEMA = (
    """CREATE TABLE reservations (
        ...
        UNIQUE (operator_id, candidate_record_id, prerequisite_id))""",
...
class WorkerActivationRuntimeAdmissionStoreError(RuntimeError):
...
class WorkerActivationRuntimeAdmissionStore:
    def __init__(
        self,
        ...
        *,
        ...
        for value, maximum in (
            ...
            if type(value) is not int or not 0 <= value <= maximum:
                raise WorkerActivationRuntimeAdmissionStoreError("invalid_request")
        ...
        with self._connect() as connection:
            ...
            if actual:
                if actual != self._schema or marker != 54:
                    raise WorkerActivationRuntimeAdmissionStoreError("store_corrupt")
            else:
                if marker != 0:
                    raise WorkerActivationRuntimeAdmissionStoreError("store_corrupt")
                for statement in _SCHEMA:
                    connection.execute(statement)
                connection.execute("PRAGMA application_id=54")
    ...
    def _connect(self) -> Iterator[sqlite3.Connection]:
        ...
        try:
            ...
            if self.max_database_bytes < size:
                raise WorkerActivationRuntimeAdmissionStoreError("quota_exceeded")
            connection.execute(
            ...
            raise WorkerActivationRuntimeAdmissionStoreError(code) from None
        finally:
    ...
    def _check_integrity(self, connection):
        try:
            if (
                ...
                != self._schema
                or connection.execute("PRAGMA application_id").fetchone()[0] != 54
            ...
            for row in connection.execute("SELECT * FROM reservations"):
                reservation = self._decode(
                    c.WorkerActivationRuntimeAdmissionSubjectReservationV1, row[6]
                )
                if row[:6] != (
            ...
            for subject, record_json, audit_json in connection.execute(
                ...
                record = self._decode(c.WorkerActivationRuntimeAdmissionV1, record_json)
                audit = self._decode(
                    c.WorkerActivationRuntimeAdmissionAuditEvidenceV1, audit_json
                )
    ...
    def _validate_record(record, reservation):
        create = c.build_create(
            ...
            receipt_status=record.worker_activation_runtime_prerequisite_status,
        )
        if (
            record.operator_id != reservation.operator_id
            or record.candidate_record_id != reservation.candidate_record_id
            or record.prerequisite_id != reservation.prerequisite_id
    ...
    def _validate_audit(audit, reservation, record):
        if (
            ...
            or audit.runtime_admission_record_fingerprint
            != (
    ...
    def _resolve(self, connection, operator_id, idem, request):
        ...
        if row is None:
            return None
        if row[1] != request:
            raise WorkerActivationRuntimeAdmissionStoreError("idempotency_conflict")
        evidence = connection.execute(
            "SELECT record_json FROM evidence WHERE subject=?", (row[0],)
        ).fetchone()
        if evidence is None:
            raise WorkerActivationRuntimeAdmissionStoreError("append_indeterminate")
        return self._decode(c.WorkerActivationRuntimeAdmissionV1, evidence[0])
    ...
    def append(
        ...
        *,
        ...
        try:
            reservation = (
                c.WorkerActivationRuntimeAdmissionSubjectReservationV1.model_validate(
                    reservation
                )
            )
            ...
            with self._connect() as connection:
                ...
                if (
                    ...
                    raise WorkerActivationRuntimeAdmissionStoreError("quota_exceeded")
                record = c.WorkerActivationRuntimeAdmissionV1.model_validate(prepare())
                self._validate_record(record, reservation)
        ...
        except WorkerActivationRuntimeAdmissionStoreError:
    ...
    def _audit(reservation, correlation, record):
        raw = {
            ...
            "worker_activation_runtime_admission_recorded": record is not None,
        }
        return c._signed(
    ...
    def _bounded(self, model):
        if len(model.model_dump_json().encode()) > self.max_model_bytes:
            raise WorkerActivationRuntimeAdmissionStoreError("record_too_large")

    def get(self, *, operator_id, candidate_record_id, runtime_admission_id):
        with self._connect() as connection:
            for (payload,) in connection.execute(
                ...
                record = self._decode(c.WorkerActivationRuntimeAdmissionV1, payload)
                if record.runtime_admission_id == runtime_admission_id:
                    return record
        raise WorkerActivationRuntimeAdmissionStoreError("evidence_not_found")

    def list_owned(self, *, operator_id, candidate_record_id):
        with self._connect() as connection:
            return tuple(
                self._decode(c.WorkerActivationRuntimeAdmissionV1, row[0])
