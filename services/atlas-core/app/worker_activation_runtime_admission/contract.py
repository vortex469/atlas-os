"""Pure v0.54 Worker Activation Runtime Admission evidence contract.

All facts are injected. No persistence, clock reads, queue/worker contact or
runtime effects occur here. The complete immutable v0.53 pair preserves lineage.
"""
...
PERMISSION = "installation.execution.worker_activation_runtime_admission.evaluate"
SCOPE = "worker_activation_runtime_admission_only"
MARKER = "worker_activation_runtime_admission_recorded"
SUCCESS_BLOCKERS = v053.SUCCESS_BLOCKERS
SAFE_MESSAGE = "worker activation runtime admission request could not be completed"
...
class ClosedAuthorityV1(ContractModel, v053.ClosedAuthorityV1):
...
class WorkerActivationRuntimeAdmissionCreateV1(ClosedAuthorityV1):
    schema: Literal["worker-activation-runtime-admission-create-v1"] = (
        "worker-activation-runtime-admission-create-v1"
    )
    ...
    @model_validator(mode="after")
    def bounded(self) -> WorkerActivationRuntimeAdmissionCreateV1:
        if len(canonical_json(self, max_depth=MAX_CREATE_NESTING)) > MAX_CREATE_BYTES:
            raise ValueError("create envelope exceeds bound")
        return self


class WorkerActivationRuntimeAdmissionAuthorityContextV1(ClosedAuthorityV1):
    schema: Literal["worker-activation-runtime-admission-authority-context-v1"] = (
        "worker-activation-runtime-admission-authority-context-v1"
    )
...
def _validate_pair(
    record: v053.WorkerActivationRuntimePrerequisiteV1,
    status: v053.WorkerActivationRuntimePrerequisiteStatusV1,
    *,
    operator_id: str,
    candidate_record_id: str,
    create: WorkerActivationRuntimeAdmissionCreateV1,
    now: str,
) -> None:
    ...
    if (
        create.prerequisite_id != record.prerequisite_id
        or status.prerequisite_id != record.prerequisite_id
        or create.valid_until != record.valid_until
        or status.valid_until != record.valid_until
    ):
        raise _Refusal("linkage_mismatch")
    if (
        create.prerequisite_record_fingerprint != record.prerequisite_record_fingerprint
        ...
        != v053.prerequisite_record_fingerprint(record)
        or status.status_fingerprint != v053.status_fingerprint(status)
    ):
        raise _Refusal("fingerprint_mismatch")
    if status.lifecycle != "active":
        raise _Refusal("v053_prerequisite_not_active")
    instant = v053.v052._instant(now)
    if any(
        v053.v052._instant(start) > instant
        or instant - v053.v052._instant(start)
    ...
    if status != v053.derive_status(record, evaluated_at=status.evaluated_at):
...
class WorkerActivationRuntimeAdmissionValidationInputV1(ContractModel):
    operator_id: OperatorId
    candidate_record_id: CanonicalUuid4
    authority: WorkerActivationRuntimeAdmissionAuthorityContextV1
    create: WorkerActivationRuntimeAdmissionCreateV1
    worker_activation_runtime_prerequisite: v053.WorkerActivationRuntimePrerequisiteV1
    worker_activation_runtime_prerequisite_status: (
        v053.WorkerActivationRuntimePrerequisiteStatusV1
    )
    ...
    @model_validator(mode="after")
    def exact(self) -> WorkerActivationRuntimeAdmissionValidationInputV1:
        if self.home_assistant:
            raise _Refusal("installation_capability_unsupported")
        if self.operator_id != self.authority.authenticated_operator_id:
            raise _Refusal("evidence_not_found")
        _validate_pair(
            self.worker_activation_runtime_prerequisite,
            self.worker_activation_runtime_prerequisite_status,
...
def fingerprint(kind: str, value: Any) -> FingerprintV1:
    return v053.v052.fingerprint(
        f"atlas:worker-activation-runtime-admission-{kind}:v1", value
    )
...
def runtime_admission_record_fingerprint(value: BaseModel | dict) -> FingerprintV1:
    return _model_fingerprint("record", value, "runtime_admission_record_fingerprint")
...
def subject_fingerprint(
    *, operator_id: str, candidate_record_id: str, prerequisite_id: str
) -> FingerprintV1:
...
def idempotency_key_fingerprint(operator_id: str, key: str) -> FingerprintV1:
    return fingerprint(
        "idempotency-key", {"operator_id": operator_id, "key": v053.v052._visible(key)}
    )
...
def request_fingerprint(
    *,
    ...
    create = WorkerActivationRuntimeAdmissionCreateV1.model_validate(create)
    return fingerprint(
        "request",
        {
            "operator_id": operator_id,
...
def derived_runtime_admission_id(subject: FingerprintV1) -> str:
    return v053.v052.derived_uuid5(
        "atlas:worker-activation-runtime-admission-id:v1", subject
    )


class WorkerActivationRuntimeAdmissionEvaluationV1(ClosedAuthorityV1):
    schema: Literal["worker-activation-runtime-admission-evaluation-v1"] = (
        "worker-activation-runtime-admission-evaluation-v1"
    )
    operator_id: OperatorId
    candidate_record_id: CanonicalUuid4
    ...
    blockers: tuple[v053.v052.BlockerV1 | RefusalV1, ...]
    recognized_v053_prerequisite_count: int
    worker_activation_runtime_admission_recorded: bool
    evaluation_fingerprint: FingerprintV1

    @model_validator(mode="after")
    def exact(self) -> WorkerActivationRuntimeAdmissionEvaluationV1:
        success = self.admission_state == "recorded"
        if (
            self.worker_activation_runtime_admission_recorded != success
            or self.eligibility != (MARKER if success else "blocked")
            or self.recognized_v053_prerequisite_count != int(success)
        ):
        ...
        if success:
            ...
            if not (
                v053.v052._instant(self.evaluated_at)
                < v053.v052._instant(self.earliest_expiry)
                <= v053.v052._instant(self.evaluated_at) + timedelta(seconds=30)
            ):
...
def evaluate_worker_activation_runtime_admission(
    value: Any,
) -> WorkerActivationRuntimeAdmissionEvaluationV1:
    ...
    try:
        ...
        if not isinstance(raw, dict) or any(
            ...
            raise _Refusal(code)
        validation = WorkerActivationRuntimeAdmissionValidationInputV1.model_validate(
            raw
        )
        ...
        return _signed(
            WorkerActivationRuntimeAdmissionEvaluationV1,
            {
                ...
                "eligibility": "blocked",
                "blockers": (code,),
                "recognized_v053_prerequisite_count": 0,
    ...
    return _signed(
        WorkerActivationRuntimeAdmissionEvaluationV1,
        {
            ...
            "recognized_v053_prerequisite_count": 1,
...
class WorkerActivationRuntimeAdmissionV1(ClosedAuthorityV1):
    schema: Literal["worker-activation-runtime-admission-v1"] = (
        "worker-activation-runtime-admission-v1"
    )
    ...
    blockers: tuple[v053.v052.BlockerV1, ...] = SUCCESS_BLOCKERS
    worker_activation_runtime_prerequisite: v053.WorkerActivationRuntimePrerequisiteV1
    worker_activation_runtime_prerequisite_status: (
        v053.WorkerActivationRuntimePrerequisiteStatusV1
    )
    ...
    @model_validator(mode="after")
    def exact(self) -> WorkerActivationRuntimeAdmissionV1:
        receipt = self.worker_activation_runtime_prerequisite
        status = self.worker_activation_runtime_prerequisite_status
        _validate_pair(
            receipt,
            status,
            operator_id=self.operator_id,
        ...
        if (
            self.prerequisite_id != receipt.prerequisite_id
            or self.admission_id != receipt.admission_id
            or not self.recorded_at < self.valid_until <= receipt.valid_until
            or v053.v052._instant(self.valid_until)
            > v053.v052._instant(self.recorded_at)
            + timedelta(seconds=MAX_FRESHNESS_SECONDS)
        ...
        ) or self.runtime_admission_id != derived_runtime_admission_id(
            self.subject_fingerprint
        ):
            raise ValueError("subject or id mismatch")
        if (
            self.runtime_admission_record_fingerprint
            != runtime_admission_record_fingerprint(self)
        ):
            raise ValueError("record fingerprint mismatch")
        return self


class WorkerActivationRuntimeAdmissionStatusV1(ClosedAuthorityV1):
    schema: Literal["worker-activation-runtime-admission-status-v1"] = (
        "worker-activation-runtime-admission-status-v1"
    )
    runtime_admission_id: CanonicalUuid5
    admission_id: CanonicalUuid5
    operator_id: OperatorId
    ...
    @model_validator(mode="after")
    def exact(self) -> WorkerActivationRuntimeAdmissionStatusV1:
        if self.runtime_admission_id != derived_runtime_admission_id(
            subject_fingerprint(
                operator_id=self.operator_id,
                candidate_record_id=self.candidate_record_id,
                prerequisite_id=self.prerequisite_id,
        ...
        if (
            ...
            or not (
                v053.v052._instant(self.recorded_at)
                < v053.v052._instant(self.valid_until)
                <= v053.v052._instant(self.recorded_at) + timedelta(seconds=30)
            )
            or self.lifecycle
...
class WorkerActivationRuntimeAdmissionResultV1(ClosedAuthorityV1):
    schema: Literal["worker-activation-runtime-admission-result-v1"] = (
        "worker-activation-runtime-admission-result-v1"
    )
    record: WorkerActivationRuntimeAdmissionV1
    status: WorkerActivationRuntimeAdmissionStatusV1
    exact_duplicate: bool = False
    worker_activation_runtime_admission_recorded: Literal[True] = True

    @model_validator(mode="after")
    def exact(self) -> WorkerActivationRuntimeAdmissionResultV1:
...
class WorkerActivationRuntimeAdmissionCollectionV1(ClosedAuthorityV1):
    schema: Literal["worker-activation-runtime-admission-collection-v1"] = (
        "worker-activation-runtime-admission-collection-v1"
    )
    operator_id: OperatorId
    candidate_record_id: CanonicalUuid4
    items: tuple[WorkerActivationRuntimeAdmissionV1, ...]
    count: int
    collection_fingerprint: FingerprintV1

    @model_validator(mode="after")
    def exact(self) -> WorkerActivationRuntimeAdmissionCollectionV1:
        if self.count != len(self.items) or not 0 <= self.count <= 16:
            raise ValueError("collection bound mismatch")
        if len({item.runtime_admission_id for item in self.items}) != self.count or any(
...
class WorkerActivationRuntimeAdmissionSubjectReservationV1(ClosedAuthorityV1):
    schema: Literal["worker-activation-runtime-admission-reservation-v1"] = (
        "worker-activation-runtime-admission-reservation-v1"
    )
    operator_id: OperatorId
    candidate_record_id: CanonicalUuid4
    prerequisite_id: CanonicalUuid5
    ...
    @model_validator(mode="after")
    def exact(self) -> WorkerActivationRuntimeAdmissionSubjectReservationV1:
        if self.subject_fingerprint != subject_fingerprint(
            operator_id=self.operator_id,
            candidate_record_id=self.candidate_record_id,
            prerequisite_id=self.prerequisite_id,
...
class WorkerActivationRuntimeAdmissionAuditEvidenceV1(ClosedAuthorityV1):
    schema: Literal["worker-activation-runtime-admission-audit-v1"] = (
        "worker-activation-runtime-admission-audit-v1"
    )
    operator_id: OperatorId
    candidate_record_id: CanonicalUuid4
    ...
    runtime_admission_record_fingerprint: FingerprintV1 | None
    worker_activation_runtime_admission_recorded: bool
    audit_fingerprint: FingerprintV1

    @model_validator(mode="after")
    def exact(self) -> WorkerActivationRuntimeAdmissionAuditEvidenceV1:
        success = self.outcome == "recorded"
        if (
            self.worker_activation_runtime_admission_recorded != success
            or (self.runtime_admission_record_fingerprint is not None) != success
...
class WorkerActivationRuntimeAdmissionRedactedErrorV1(ClosedAuthorityV1):
    schema: Literal["worker-activation-runtime-admission-error-v1"] = (
        "worker-activation-runtime-admission-error-v1"
    )
    error_code: (
        RefusalV1
        | Literal[
    ...
    worker_activation_runtime_admission_recorded: Literal[False] = False


def build_create(
    *,
    receipt: v053.WorkerActivationRuntimePrerequisiteV1,
    receipt_status: v053.WorkerActivationRuntimePrerequisiteStatusV1,
) -> WorkerActivationRuntimeAdmissionCreateV1:
    return WorkerActivationRuntimeAdmissionCreateV1(
        prerequisite_id=receipt.prerequisite_id,
        valid_until=receipt.valid_until,
        prerequisite_record_fingerprint=receipt.prerequisite_record_fingerprint,
        status_fingerprint=receipt_status.status_fingerprint,
    )


def build_runtime_admission(
    validation: WorkerActivationRuntimeAdmissionValidationInputV1,
    *,
    idempotency_key: str,
) -> WorkerActivationRuntimeAdmissionV1:
    validation = WorkerActivationRuntimeAdmissionValidationInputV1.model_validate(
        validation
    )
    ...
    return _signed(
        WorkerActivationRuntimeAdmissionV1,
        {
            "runtime_admission_id": derived_runtime_admission_id(subject),
            "admission_id": validation.worker_activation_runtime_prerequisite.admission_id,
            "operator_id": validation.operator_id,
            "candidate_record_id": validation.candidate_record_id,
            "prerequisite_id": validation.create.prerequisite_id,
            ...
            "worker_activation_runtime_prerequisite": (
                validation.worker_activation_runtime_prerequisite
            ),
            "worker_activation_runtime_prerequisite_status": (
                validation.worker_activation_runtime_prerequisite_status
            ),
        ...
        "runtime_admission_record_fingerprint",
        runtime_admission_record_fingerprint,
    )


def derive_status(
    record: WorkerActivationRuntimeAdmissionV1, *, evaluated_at: str
) -> WorkerActivationRuntimeAdmissionStatusV1:
    record = WorkerActivationRuntimeAdmissionV1.model_validate(record)
    return _signed(
        WorkerActivationRuntimeAdmissionStatusV1,
        {
            **{
...
def parse_create_json(data: str | bytes) -> WorkerActivationRuntimeAdmissionCreateV1:
    try:
        ...
        return WorkerActivationRuntimeAdmissionCreateV1.model_validate(raw)
