...
def test_v049_release_closure_is_documented_as_queue_claim_admission_only() -> None:
    changelog = _read("CHANGELOG.md")
    roadmap = _read("ROADMAP.md")
    checklist = _read("docs/RELEASE_CHECKLIST.md")
    contract = _read(
        "docs/architecture/controlled-worker-queue-claim-admission-v1.md"
    )

    assert "#### v0.49 P0-P5 - Controlled Worker Queue Claim Admission" in changelog
    assert "P5 adds focused tests and release documentation only." in changelog
    assert "## Completed v0.49 plan - Controlled Worker Queue Claim Admission" in roadmap
    assert "P0-P5 are complete." in roadmap
    assert "## Atlas v0.49 P0-P5 Controlled Worker Queue Claim Admission - complete" in (
        checklist
    )
    assert (
        "Status: **Atlas v0.49 P0-P5 closed controlled worker queue claim "
        "admission contract**."
        in contract
    )
    ...
    for phrase in (
        "one exact active same-owner v0.48",
...
def test_v049_only_queue_claim_admission_advanced_and_later_authority_blocked() -> None:
    contract = _read(
        "services/atlas-core/app/controlled_worker_queue_claim_admission/contract.py"
    )
    ...
    assert "SUCCESS_BLOCKERS as V048_SUCCESS_BLOCKERS" in contract
    assert "WorkerBindingActivationEvidenceV1" in contract
    assert "WorkerBindingActivationEvidenceStatusV1" in contract
    assert "recognized_v048_activation_evidence_count" in contract
...
def test_v049_release_closure_does_not_add_effect_configuration() -> None:
    assert not ROOT.joinpath("compose.execution-smoke.override.yaml").exists()

    production_files = (
        "services/atlas-core/app/controlled_worker_queue_claim_admission/contract.py",
        "services/atlas-core/app/controlled_worker_queue_claim_admission/service.py",
        "services/atlas-core/app/controlled_worker_queue_claim_admission/store.py",
        "services/atlas-core/app/routes/controlled_worker_queue_claim_admission.py",
    )
    ...
    allowed_imports = {
        ...
        "app.controlled_worker_queue_claim_admission.service",
        "app.controlled_worker_queue_claim_admission.store",
    }
    forbidden_def_names = {
        "ack",
        "acknowledge",
...
def test_v049_default_off_api_ui_agent_and_execution_worker_isolation() -> None:
    service = _read(
        "services/atlas-core/app/controlled_worker_queue_claim_admission/service.py"
    )
    store = _read(
    ...
    assert "no queue claim, lease, ack, or worker API" in service
    assert "read_owned" in service
    assert "resolve_idempotency" in service
    assert "append_indeterminate" in service
    assert "UPDATE controlled_worker_queue_claim_admission" not in store
    assert "DELETE FROM controlled_worker_queue_claim_admission" not in store
    assert "controlled_worker_queue_claim_admission_service" not in main
    assert "create_controlled_worker_queue_claim_admission_service" not in main

    assert '@router.post(\n    "/{candidate_record_id}/controlled-worker-queue-claim-admissions"' in route
    assert '@router.get(\n    "/{candidate_record_id}/controlled-worker-queue-claim-admissions"' in route
    assert (
        '@router.get(\n    "/{candidate_record_id}/controlled-worker-queue-claim-admissions/'
        '{admission_id}"'
        in route
    )
    ...
    assert "v0.49 controlled worker queue claim admission Mission Control boundary" in (
        mission_control_structural
    )
    assert r"atlas\.(post|put|patch|delete)" in mission_control_structural
    assert "controlledWorkerQueueClaimAdmission|ControlledWorkerQueueClaimAdmission" in (
        mission_control_structural
    )
