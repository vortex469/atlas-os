...
class RuntimeResourceLimitsV1(ContractModel):
    schema: Literal["worker-activation-runtime-resource-limits-v1"] = (
        "worker-activation-runtime-resource-limits-v1"
    )
...
class RuntimeFilesystemPolicyV1(ContractModel):
    schema: Literal["worker-activation-runtime-filesystem-policy-v1"] = (
        "worker-activation-runtime-filesystem-policy-v1"
    )
    ...
    def bounded_paths(self) -> RuntimeFilesystemPolicyV1:
        ...
        if len(set(self.writable_paths)) != len(self.writable_paths) or len(
            set(self.mount_paths)
        ) != len(self.mount_paths):
            raise ValueError("filesystem paths must be unique")
        return self


class RuntimeNetworkPolicyV1(ContractModel):
    schema: Literal["worker-activation-runtime-network-policy-v1"] = (
        "worker-activation-runtime-network-policy-v1"
    )
    ...
    def exact_policy(self) -> RuntimeNetworkPolicyV1:
        ...
        if (
            any(port < 1 or port > 65535 for port in self.egress_ports)
            or len(self.egress_ports) > MAX_ITEMS
        ):
            raise ValueError("invalid network port policy")
        if len(set(self.egress_hosts)) != len(self.egress_hosts) or len(
            set(self.egress_ports)
        ) != len(self.egress_ports):
            raise ValueError("network policy entries must be unique")
        return self


class WorkerActivationRuntimeDefinitionV1(ContractModel):
    schema: Literal["worker-activation-runtime-definition-v1"] = (
    ...
    def exact(self) -> WorkerActivationRuntimeDefinitionV1:
        if (
            ...
            or len(self.capabilities) > MAX_ITEMS
...
class WorkerActivationRuntimeDefinitionCreateV1(ContractModel):
    schema: Literal["worker-activation-runtime-definition-create-v1"] = (
        "worker-activation-runtime-definition-create-v1"
    )
    definition_id: CanonicalUuid5
    definition_fingerprint: FingerprintV1
    valid_until: UtcSecond
...
class WorkerActivationRuntimeDefinitionEvaluationV1(ContractModel):
    schema: Literal["worker-activation-runtime-definition-evaluation-v1"] = (
        "worker-activation-runtime-definition-evaluation-v1"
    )
    definition_id: CanonicalUuid5
    ...
    def exact_evaluation(self) -> WorkerActivationRuntimeDefinitionEvaluationV1:
        if (self.state == "recorded") != (self.eligibility == MARKER) or (
            self.state == "blocked"
...
class WorkerActivationRuntimeDefinitionV1Record(ContractModel):
    ...
    schema: Literal["worker-activation-runtime-definition-record-v1"] = (
        "worker-activation-runtime-definition-record-v1"
    )
    definition_id: CanonicalUuid5
    operator_id: OperatorId
    candidate_record_id: CanonicalUuid4
    ...
    def exact_record(self) -> WorkerActivationRuntimeDefinitionV1Record:
        if (
            ...
            != (
                ...
                self.definition.operator_id,
                self.definition.candidate_record_id,
                self.definition.valid_until,
            )
            or self.evaluation.definition_id != self.definition_id
            or self.evaluation.definition_fingerprint
            != self.definition.definition_fingerprint
        ):
            raise ValueError("definition record linkage mismatch")
        if self.subject_fingerprint != subject_fingerprint(
            self.operator_id, self.candidate_record_id, self.definition_id
        ):
...
class WorkerActivationRuntimeDefinitionStatusV1(ContractModel):
    schema: Literal["worker-activation-runtime-definition-status-v1"] = (
        "worker-activation-runtime-definition-status-v1"
    )
    definition_id: CanonicalUuid5
    operator_id: OperatorId
    candidate_record_id: CanonicalUuid4
...
class WorkerActivationRuntimeDefinitionResultV1(ContractModel):
    schema: Literal["worker-activation-runtime-definition-result-v1"] = (
        "worker-activation-runtime-definition-result-v1"
    )
...
class WorkerActivationRuntimeDefinitionCollectionV1(ContractModel):
    schema: Literal["worker-activation-runtime-definition-collection-v1"] = (
        "worker-activation-runtime-definition-collection-v1"
    )
    operator_id: OperatorId
    candidate_record_id: CanonicalUuid4
    ...
    def exact_collection(self) -> WorkerActivationRuntimeDefinitionCollectionV1:
        if (
            ...
            or self.count > MAX_ITEMS
            or len({x.definition_id for x in self.items}) != self.count
        ):
            raise ValueError("definition collection bound mismatch")
        if any(
            x.operator_id != self.operator_id
            or x.candidate_record_id != self.candidate_record_id
            for x in self.items
        ) or self.collection_fingerprint != collection_fingerprint(self):
...
class WorkerActivationRuntimeDefinitionSubjectReservationV1(ContractModel):
    schema: Literal["worker-activation-runtime-definition-reservation-v1"] = (
        "worker-activation-runtime-definition-reservation-v1"
    )
    operator_id: OperatorId
    candidate_record_id: CanonicalUuid4
    definition_id: CanonicalUuid5
    ...
    def exact_reservation(
        self,
    ) -> WorkerActivationRuntimeDefinitionSubjectReservationV1:
        if self.subject_fingerprint != subject_fingerprint(
            self.operator_id, self.candidate_record_id, self.definition_id
...
class WorkerActivationRuntimeDefinitionAuditV1(ContractModel):
    schema: Literal["worker-activation-runtime-definition-audit-v1"] = (
        "worker-activation-runtime-definition-audit-v1"
    )
    operator_id: OperatorId
    candidate_record_id: CanonicalUuid4
...
class WorkerActivationRuntimeDefinitionErrorV1(ContractModel):
    schema: Literal["worker-activation-runtime-definition-error-v1"] = (
        "worker-activation-runtime-definition-error-v1"
    )
    error_code: Literal[
        "unauthenticated",
        "forbidden",
...
def subject_fingerprint(
    operator_id: str, candidate_record_id: str, definition_id: str
) -> FingerprintV1:
    return _fingerprint(
        "subject",
        {
...
def request_fingerprint(
    ...
    create: WorkerActivationRuntimeDefinitionCreateV1,
) -> FingerprintV1:
    return _fingerprint(
        "request",
        {
...
WorkerActivationRuntimeDefinitionAuditEvidenceV1 = (
    WorkerActivationRuntimeDefinitionAuditV1
)
...
def _fingerprint(kind: str, value: Any) -> FingerprintV1:
    ...
    digest = hashlib.sha256(
        (f"atlas:worker-activation-runtime-definition-{kind}:v1\0").encode()
...
def definition_fingerprint(
    value: WorkerActivationRuntimeDefinitionV1 | dict[str, Any],
) -> FingerprintV1:
    raw = _plain(value)
    ...
    raw["resources"] = RuntimeResourceLimitsV1.model_validate(
        raw["resources"]
    ).model_dump(mode="json")
    raw["filesystem"] = RuntimeFilesystemPolicyV1.model_validate(
        raw["filesystem"]
    ).model_dump(mode="json")
    raw["network"] = RuntimeNetworkPolicyV1.model_validate(raw["network"]).model_dump(
        mode="json"
    )
...
def evaluation_fingerprint(
    value: WorkerActivationRuntimeDefinitionEvaluationV1 | dict[str, Any],
) -> FingerprintV1:
    raw = (
        value.model_dump(mode="json")
        if isinstance(value, BaseModel)
        else WorkerActivationRuntimeDefinitionEvaluationV1.model_construct(
            **value
        ).model_dump(mode="json")
    )
...
def evaluate_worker_activation_runtime_definition(
    value: Any, *, evaluated_at: str | None = None
) -> WorkerActivationRuntimeDefinitionEvaluationV1:
    ...
    try:
        ...
        result = {
            ...
            "definition_fingerprint": FingerprintV1(
                algorithm="sha256", canonicalization="atlas-jcs-nfc-v1", value="0" * 64
            ),
        }
    seed = WorkerActivationRuntimeDefinitionEvaluationV1.model_construct(**result)
    return WorkerActivationRuntimeDefinitionEvaluationV1.model_validate(
        {**_plain(seed), "evaluation_fingerprint": evaluation_fingerprint(seed)}
    )
