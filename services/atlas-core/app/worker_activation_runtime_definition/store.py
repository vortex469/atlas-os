...
class WorkerActivationRuntimeDefinitionStore:
    def __init__(
        ...
        *,
        ...
        if any(
            type(v) is not int or v < 0 or v > limit
            for v, limit in (
                (max_records_per_operator, MAX_RECORDS_PER_OPERATOR),
                (max_total_records, MAX_TOTAL_RECORDS),
                (max_model_bytes, c.MAX_MODEL_BYTES),
        ...
        self.max_records_per_operator, self.max_total_records = (
            max_records_per_operator,
            max_total_records,
        )
        self.max_model_bytes, self.max_database_bytes = (
            max_model_bytes,
            max_database_bytes,
        )
        ...
        with self._connect(create=True) as db:
            ...
            if not tables:
                db.executescript(
                    "CREATE TABLE reservations(subject TEXT PRIMARY KEY, operator_id TEXT NOT NULL, candidate_record_id TEXT NOT NULL, definition_id TEXT NOT NULL, idem TEXT NOT NULL, request TEXT NOT NULL, reservation_json TEXT NOT NULL, UNIQUE(operator_id, idem), UNIQUE(operator_id, candidate_record_id, definition_id)); CREATE TABLE evidence(subject TEXT PRIMARY KEY REFERENCES reservations(subject), record_json TEXT NOT NULL, audit_json TEXT NOT NULL); CREATE TABLE failures(subject TEXT PRIMARY KEY REFERENCES reservations(subject), audit_json TEXT NOT NULL);"
                )
    ...
    def _check(self, db):
        try:
            if (
                ...
                or db.execute("PRAGMA application_id").fetchone()[0] != 64
            ...
            for row in db.execute(
                "SELECT subject, operator_id, candidate_record_id, definition_id, idem, request, reservation_json FROM reservations"
            ):
                reservation = self._decode(
                    c.WorkerActivationRuntimeDefinitionSubjectReservationV1, row[6]
                )
                if row[:6] != (
                    ...
                    raise ValueError
            for subject, payload, audit in db.execute(
                "SELECT subject, record_json, audit_json FROM evidence"
            ):
                record = self._decode(
                    c.WorkerActivationRuntimeDefinitionV1Record, payload
                )
                self._decode(c.WorkerActivationRuntimeDefinitionAuditV1, audit)
                if record.subject_fingerprint.value != subject:
                    raise ValueError
            for subject, audit in db.execute(
                "SELECT subject, audit_json FROM failures"
            ):
                self._decode(c.WorkerActivationRuntimeDefinitionAuditV1, audit)
    ...
    def append(self, reservation, prepare, revalidate, correlation):
        ...
        try:
            with self._connect() as db:
                ...
                if row is not None:
                    if row[1] != reservation.request_fingerprint.value:
                        raise WorkerActivationRuntimeDefinitionStoreError(
                            "idempotency_conflict"
                        )
                    ...
                    return self._decode(
                        c.WorkerActivationRuntimeDefinitionV1Record, existing_row[0]
                    ), False
                if db.execute(
                ...
                if (
                    ...
                    raise WorkerActivationRuntimeDefinitionStoreError("quota_exceeded")
                record = c.WorkerActivationRuntimeDefinitionV1Record.model_validate(
                    prepare()
                )
                audit = _audit(reservation, correlation, record)
            ...
            if attempted:
                self._failure(reservation, correlation)
            raise
        # Persistence failures after reservation are indeterminate by design;
        # hide implementation details and preserve the fail-closed contract.
    ...
    def _failure(self, reservation, correlation):
        try:
            with self._connect() as db:
                if not db.execute(
                    "SELECT 1 FROM evidence WHERE subject=?",
                    (reservation.subject_fingerprint.value,),
                ).fetchone():
                    db.execute(
                        ...
                        (
                            reservation.subject_fingerprint.value,
                            _audit(reservation, correlation, None).model_dump_json(),
                        ),
                    )
        # Failure journaling is best effort and must not mask the original
        # append error or turn the evidence boundary into an authority path.
    ...
    def get(self, operator_id, candidate_record_id, definition_id):
        with self._connect() as db:
            row = db.execute(
                "SELECT record_json FROM evidence WHERE subject IN (SELECT subject FROM reservations WHERE operator_id=? AND candidate_record_id=? AND definition_id=?)",
                (operator_id, candidate_record_id, definition_id),
            ).fetchone()
            if row:
                return self._decode(c.WorkerActivationRuntimeDefinitionV1Record, row[0])
    ...
    def list_owned(self, operator_id, candidate_record_id):
        with self._connect() as db:
            return tuple(
                self._decode(c.WorkerActivationRuntimeDefinitionV1Record, row[0])
                for row in db.execute(
                    "SELECT record_json FROM evidence JOIN reservations USING(subject) WHERE operator_id=? AND candidate_record_id=? ORDER BY subject",
...
class _Connection:
    def __init__(self, path, mode, store):
        self.db = sqlite3.connect(
            f"file:{path}?mode={mode}", uri=True, timeout=5, isolation_level=None
        )
        self.store = store
        self.db.execute("PRAGMA foreign_keys=ON")
        self.db.execute("PRAGMA synchronous=FULL")
        size = self.db.execute("PRAGMA page_size").fetchone()[0]
        if self.store.max_database_bytes < size:
            raise WorkerActivationRuntimeDefinitionStoreError("quota_exceeded")
        self.db.execute(
            f"PRAGMA max_page_count={self.store.max_database_bytes // size}"
        )
...
def _audit(reservation, correlation, record):
    raw = {
        ...
        "outcome": "recorded" if record else "indeterminate",
        "subject_fingerprint": reservation.subject_fingerprint,
        "correlation_fingerprint": correlation,
        "record_fingerprint": record.record_fingerprint if record else None,
    }
    return c.WorkerActivationRuntimeDefinitionAuditV1.model_validate(
        {**raw, "audit_fingerprint": c.audit_fingerprint(raw)}
    )
