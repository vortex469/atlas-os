...
def test_default_off_and_restart_safe_idempotency(tmp_path):
    ...
    disabled = service.create(
        _create(value), authenticated_operator_id=value.operator_id,
        permission_verified=True, candidate_record_id=value.candidate_record_id,
        idempotency_key="one", correlation_id="private",
    )
...
def test_subject_reservation_is_permanent(tmp_path):
    ...
    store = WorkerActivationRuntimeDefinitionStore(tmp_path / "definitions.sqlite")
    service = WorkerActivationRuntimeDefinitionService(
        definition_reader=Reader(value), store=store,
        clock=lambda: datetime(2026, 1, 1, tzinfo=UTC), enabled=True,
    )
    common = dict(
        authenticated_operator_id=value.operator_id, permission_verified=True,
        candidate_record_id=value.candidate_record_id, correlation_id="private",
    )
    assert isinstance(service.create(_create(value), idempotency_key="one", **common), c.WorkerActivationRuntimeDefinitionResultV1)
