...
class WorkerActivationRuntimePlanReader(Protocol):
    def read_owned(
        self,
        *,
        operator_id: str,
        ...
        tuple[
            c.v054.WorkerActivationRuntimeAdmissionV1,
            c.v054.WorkerActivationRuntimeAdmissionStatusV1,
        ]
        | None
    ): ...
...
class WorkerActivationRuntimePlanService:
    def __init__(
        self,
        *,
        prerequisite_reader: WorkerActivationRuntimePlanReader,
        store: WorkerActivationRuntimePlanStore,
    ...
    def create(
        ...
        *,
        ...
        try:
            ...
            create = c.WorkerActivationRuntimePlanCreateV1.model_validate(create)
            ...
            reservation = c._signed(
                c.WorkerActivationRuntimePlanSubjectReservationV1,
                {
                    ...
                    "runtime_admission_id": create.runtime_admission_id,
            ...
            def validate():
                ...
                if now < last_validated_at:
                    raise WorkerActivationRuntimePlanStoreError("evidence_stale")
                ...
                raw = {
                    ...
                    "create": create,
                    "worker_activation_runtime_admission": pair[0],
                    "worker_activation_runtime_admission_status": pair[1],
                }
                evaluation = c.evaluate_worker_activation_runtime_plan(raw)
                if not evaluation.worker_activation_runtime_plan_recorded:
                    raise WorkerActivationRuntimePlanStoreError(evaluation.blockers[0])
                raw["authority"] = c.WorkerActivationRuntimePlanAuthorityContextV1(
                    authenticated_operator_id=operator,
                    permission=c.PERMISSION,
                    permission_verified=True,
                    request_received_at=received_at,
                )
                return c.WorkerActivationRuntimePlanValidationInputV1.model_validate(
                    raw
                )

            def prepare():
                return c.build_runtime_plan(validate(), idempotency_key=idempotency_key)

            def revalidate(record):
                current = prepare()
                if current != record:
                    raise WorkerActivationRuntimePlanStoreError("fingerprint_mismatch")
    ...
    def _authorize(operator, permission):
        if operator is None:
            raise WorkerActivationRuntimePlanStoreError("unauthenticated")
        if permission is not True:
            raise WorkerActivationRuntimePlanStoreError("forbidden")
        return TypeAdapter(c.OperatorId).validate_python(operator, strict=True)

    def _result(self, record, duplicate):
        return c.WorkerActivationRuntimePlanResultV1(
            record=record,
    ...
    def _failure(cls, code, correlation):
        return c.WorkerActivationRuntimePlanRedactedErrorV1(
            error_code=code, correlation_fingerprint=cls._correlation(correlation)
        )


def create_worker_activation_runtime_plan_service(**kwargs):
    """Explicit opt-in composition only; never called by production startup."""
    return WorkerActivationRuntimePlanService(**kwargs)
