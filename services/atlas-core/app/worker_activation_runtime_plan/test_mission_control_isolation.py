"""The v0.55 Mission Control consumer is nested evidence display only."""
...
def test_ui_reuses_the_exact_predecessor_false_authority_ceiling() -> None:
    from app.worker_activation_runtime_plan.contract import ClosedAuthorityV1
    ...
    source = (ui / "api/workerActivationRuntimePlan.ts").read_text()
    ...
    types = (ui / "types/workerActivationRuntimePlan.ts").read_text()
    assert "extends WorkerActivationRuntimeAdmission" in types
