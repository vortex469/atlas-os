...
_SCHEMA = (
    """CREATE TABLE reservations (
        ...
        UNIQUE (operator_id, candidate_record_id, runtime_admission_id))""",
...
class WorkerActivationRuntimePlanStoreError(RuntimeError):
...
class WorkerActivationRuntimePlanStore:
    def __init__(
        self,
        ...
        *,
        ...
        for value, maximum in (
            ...
            if type(value) is not int or not 0 <= value <= maximum:
                raise WorkerActivationRuntimePlanStoreError("invalid_request")
        ...
        with self._connect() as connection:
            ...
            if actual:
                if actual != self._schema or marker != 55:
                    raise WorkerActivationRuntimePlanStoreError("store_corrupt")
            else:
                if marker != 0:
                    raise WorkerActivationRuntimePlanStoreError("store_corrupt")
                for statement in _SCHEMA:
                    connection.execute(statement)
                connection.execute("PRAGMA application_id=55")
    ...
    def _connect(self) -> Iterator[sqlite3.Connection]:
        ...
        try:
            ...
            if self.max_database_bytes < size:
                raise WorkerActivationRuntimePlanStoreError("quota_exceeded")
            connection.execute(
            ...
            raise WorkerActivationRuntimePlanStoreError(code) from None
        finally:
    ...
    def _check_integrity(self, connection):
        try:
            if (
                ...
                != self._schema
                or connection.execute("PRAGMA application_id").fetchone()[0] != 55
            ...
            for subject, record_json, audit_json in connection.execute(
                ...
                record = self._decode(c.WorkerActivationRuntimePlanV1, record_json)
                audit = self._decode(
                    c.WorkerActivationRuntimePlanAuditEvidenceV1, audit_json
                )
    ...
    def _validate_record(record, reservation):
        create = c.build_create(
            ...
            receipt_status=record.worker_activation_runtime_admission_status,
        )
        if (
            record.operator_id != reservation.operator_id
            or record.candidate_record_id != reservation.candidate_record_id
            or record.runtime_admission_id != reservation.runtime_admission_id
    ...
    def _validate_audit(audit, reservation, record):
        if (
            ...
            or audit.runtime_plan_record_fingerprint
            != (record.runtime_plan_record_fingerprint if record is not None else None)
        ):
    ...
    def _resolve(self, connection, operator_id, idem, request):
        ...
        if row is None:
            return None
        if row[1] != request:
            raise WorkerActivationRuntimePlanStoreError("idempotency_conflict")
        evidence = connection.execute(
            "SELECT record_json FROM evidence WHERE subject=?", (row[0],)
        ).fetchone()
        if evidence is None:
            raise WorkerActivationRuntimePlanStoreError("append_indeterminate")
        return self._decode(c.WorkerActivationRuntimePlanV1, evidence[0])
    ...
    def append(
        ...
        *,
        ...
        try:
            reservation = (
                c.WorkerActivationRuntimePlanSubjectReservationV1.model_validate(
                    reservation
                )
            )
            ...
            with self._connect() as connection:
                ...
                if (
                    ...
                    raise WorkerActivationRuntimePlanStoreError("quota_exceeded")
                record = c.WorkerActivationRuntimePlanV1.model_validate(prepare())
                self._validate_record(record, reservation)
        ...
        except WorkerActivationRuntimePlanStoreError:
    ...
    def _bounded(self, model):
        if len(model.model_dump_json().encode()) > self.max_model_bytes:
            raise WorkerActivationRuntimePlanStoreError("record_too_large")

    def get(self, *, operator_id, candidate_record_id, runtime_plan_id):
        with self._connect() as connection:
            for (payload,) in connection.execute(
                ...
                record = self._decode(c.WorkerActivationRuntimePlanV1, payload)
                if record.runtime_plan_id == runtime_plan_id:
                    return record
        raise WorkerActivationRuntimePlanStoreError("evidence_not_found")

    def list_owned(self, *, operator_id, candidate_record_id):
        with self._connect() as connection:
            return tuple(
                self._decode(c.WorkerActivationRuntimePlanV1, row[0])
