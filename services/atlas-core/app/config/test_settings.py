...
def test_operator_auth_is_disabled_safely_by_default() -> None:
    ...
    assert OperatorAuthSettings().worker_admission_stub_database == (
...
@pytest.mark.parametrize("database", [":memory:", "relative/intents.db", ""])
def test_installation_approval_intent_database_requires_durable_absolute_path(
    database: str,
) -> None:
    with pytest.raises(ValidationError, match="absolute"):
...
def test_installation_execution_admission_database_requires_durable_absolute_path(
    database: str,
) -> None:
    with pytest.raises(ValidationError, match="absolute"):
...
@pytest.mark.parametrize("database", [":memory:", "relative/stubs.db", ""])
def test_worker_admission_stub_database_requires_durable_absolute_path(
    database: str,
) -> None:
    with pytest.raises(ValidationError, match="absolute"):
        OperatorAuthSettings(worker_admission_stub_database=database)


@pytest.mark.parametrize("database", [":memory:", "relative/reservations.db", ""])
def test_worker_queue_reservation_database_requires_durable_absolute_path(
    database: str,
) -> None:
    with pytest.raises(ValidationError, match="absolute"):
