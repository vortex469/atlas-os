...
from app.worker_binding_activation_evidence import service, store
...
from app.worker_binding_activation_evidence.service import (
    create_worker_binding_activation_evidence_service,
)
from app.worker_binding_activation_evidence.store import (
    WorkerBindingActivationEvidenceStore,
)
from app.worker_binding_activation_evidence.test_contract import _facts
...
def _service(
    ...
    *,
    ...
    preflight_reader = Reader(
        preflight_evidence if preflight_evidence is not None else (binding, binding_status)
    )
    activation_evidence_store = WorkerBindingActivationEvidenceStore(
        tmp_path / "worker-binding-activation-evidence.sqlite3",
        max_records_per_operator=quota,
        max_model_bytes=max_model_bytes,
    )
    activation_evidence_service = create_worker_binding_activation_evidence_service(
        preflight_reader=preflight_reader,
        store=activation_evidence_store,
        clock=_clock(second),
        enabled=enabled,
    )
    return (
        activation_evidence_service,
        activation_evidence_store,
        preflight_reader,
        binding,
        binding_status,
        create,
...
def _record(activation_evidence_service, binding, create, **changes):
    values = {
        ...
        "correlation_id": "worker-binding-activation-evidence-correlation-1",
    }
    values.update(changes)
    return activation_evidence_service.create(create, **values)
...
def test_create_get_list_and_restart_safe_owner_readback(tmp_path: Path) -> None:
    ...
    created = _record(activation_evidence_service, binding, create)
    assert created.ok
    assert created.record.activation_evidence_state == "readiness_gated"
    assert created.status.lifecycle == "active"
    assert preflight_reader.calls == 1
    ...
    restarted = create_worker_binding_activation_evidence_service(
        preflight_reader=Reader(None),
        store=WorkerBindingActivationEvidenceStore(activation_evidence_store.database_path),
        clock=_clock(70),
    )
    readback = restarted.get(
        authenticated_operator_id=binding.operator_id,
        permission_verified=True,
        activation_evidence_id=created.record.activation_evidence_id,
    ...
    assert tuple(item.activation_evidence_id for item in listed.items) == (
        created.record.activation_evidence_id,
    )
...
def test_default_disabled_auth_permission_missing_and_zero_effect(
    ...
    activation_evidence_service, activation_evidence_store, preflight_reader, binding, _status, create = (
        _service(tmp_path, enabled=False)
    )
    assert _record(activation_evidence_service, binding, create).error.error_code == (
        "installation_capability_unsupported"
    )
    assert _record(
        activation_evidence_service, binding, create, authenticated_operator_id=None
    ).error.error_code == "unauthenticated"
    assert _record(
        activation_evidence_service, binding, create, permission_verified=False
    ).error.error_code == "forbidden"
    assert preflight_reader.calls == 0
    assert activation_evidence_store.list_owned(
        operator_id=binding.operator_id,
        candidate_record_id=binding.candidate_record_id,
    ) == ()

    missing = _service(tmp_path / "missing", preflight_evidence=None)
...
def test_exact_duplicate_zero_reader_and_secret_free_persistence(
    ...
    duplicate_service = create_worker_binding_activation_evidence_service(
        preflight_reader=preflight_reader,
        store=WorkerBindingActivationEvidenceStore(activation_evidence_store.database_path),
        clock=_clock(70),
        enabled=True,
    ...
    with sqlite3.connect(activation_evidence_store.database_path) as connection:
        rows = []
        for table in (
            "worker_binding_activation_evidence_reservations",
            "worker_binding_activation_evidences",
            "worker_binding_activation_evidence_attempts",
        ):
            rows.extend(connection.execute(f"SELECT * FROM {table}").fetchall())
        schema_rows = connection.execute(
            "SELECT sql FROM sqlite_master WHERE name LIKE 'worker_binding_activation_evidence%'"
        ).fetchall()
    persisted = "\n".join(str(value) for row in rows + schema_rows for value in row)
    assert "worker-binding-activation-evidence-key-1" not in persisted
...
def test_idempotency_subject_conflicts_and_prerequisite_validation(
    ...
    assert _record(activation_evidence_service, binding, create).ok
    ...
    changed = create.model_copy(
        update={"preflight_valid_until": "2026-08-27T12:00:44Z"}
    )
    assert _record(activation_evidence_service, binding, changed).error.error_code == (
    ...
    assert preflight_reader.calls == 2
    assert activation_evidence_store.list_owned(
        operator_id=binding.operator_id,
        candidate_record_id=binding.candidate_record_id,
    )
...
def test_concurrent_duplicate_reservation_yields_one_durable_record(
    ...
    activation_evidence_service, activation_evidence_store, _reader, binding, _status, create = _service(
        tmp_path
    )
    with ThreadPoolExecutor(max_workers=8) as pool:
        results = list(
            pool.map(lambda _index: _record(activation_evidence_service, binding, create), range(8))
        )
    ...
    listed = activation_evidence_store.list_owned(
        operator_id=binding.operator_id,
        candidate_record_id=binding.candidate_record_id,
    )
    assert len(listed) == 1
    assert {
        result.record.activation_evidence_record_fingerprint.value
        for result in results
        if result.ok
    } == {listed[0].activation_evidence_record_fingerprint.value}


def test_indeterminate_reservation_is_terminal_across_restart(
    tmp_path: Path,
) -> None:
    _activation_evidence_service, activation_evidence_store, _reader, binding, binding_status, create = (
        _service(tmp_path)
    )
    validation = WorkerBindingActivationEvidenceValidationInputV1(
        ...
        authority=WorkerBindingActivationEvidenceAuthorityContextV1(
            ...
            request_received_at="2026-08-27T12:00:40Z",
        ),
        candidate_record_id=binding.candidate_record_id,
        create=create,
        worker_binding_activation_preflight=binding,
        worker_binding_activation_preflight_status=binding_status,
        idempotency_key="worker-binding-activation-evidence-key-1",
    )
    ...
    audit = build_audit(
        record,
        event="worker_binding_activation_evidence_indeterminate",
        outcome="indeterminate",
        correlation_fingerprint=opaque_fingerprint("atlas:test:v1", "indeterminate"),
        occurred_at=record.recorded_at,
    )
    try:
        activation_evidence_store.append(
            record=record,
            idempotency_reservation=idempotency,
            subject_reservation=reservation,
            audit_evidence=audit,
            preflight_valid_until=create.preflight_valid_until,
            force_indeterminate=True,
        )
    except store.WorkerBindingActivationEvidenceStoreError as error:
        assert error.code == "append_indeterminate"
    retry = create_worker_binding_activation_evidence_service(
        preflight_reader=Reader((binding, binding_status)),
        store=WorkerBindingActivationEvidenceStore(activation_evidence_store.database_path),
        clock=_clock(),
        enabled=True,
    )
...
def test_quota_bounds_corruption_and_append_only_surface(tmp_path: Path) -> None:
    ...
    assert not any(
        hasattr(WorkerBindingActivationEvidenceStore, name)
        for name in (
            "delete",
            "release",
            "activate",
    ...
    with sqlite3.connect(corrupt[1].database_path) as connection:
        connection.execute(
            "UPDATE worker_binding_activation_evidences SET record_json = ?", ("{}",)
        )
...
def test_service_store_have_no_effect_imports_calls_or_production_consumers() -> None:
    ...
    for path in root.rglob("*.py"):
        if path.parent.name == "worker_binding_activation_evidence":
            continue
        if path.name.startswith("test_"):
            continue
        source_text = path.read_text(encoding="utf-8")
        if (
            "worker_binding_activation_evidence.service" in source_text
            or "WorkerBindingActivationEvidenceService" in source_text
        ):
