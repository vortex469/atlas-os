...
def test_v047_release_closure_is_documented_as_preflight_only() -> None:
    changelog = _read("CHANGELOG.md")
    roadmap = _read("ROADMAP.md")
    contract = _read(
        "docs/architecture/worker-binding-activation-preflight-v1.md"
    )

    assert (
        "#### v0.47 P0-P5 - Worker Binding Activation Preflight" in changelog
    )
    ...
    assert (
        "Status: **Atlas v0.47 P0-P5 closed worker binding activation "
        "preflight contract**."
        in contract
    )
...
def test_v047_only_preflight_evidence_advanced_and_later_authority_is_blocked() -> None:
    contract = _read(
        "services/atlas-core/app/worker_binding_activation_preflight/contract.py"
    )
    ...
    assert "SUCCESS_BLOCKERS as V046_SUCCESS_BLOCKERS" in contract
    assert "OneShotDequeueWorkerBindingV1" in contract
    assert "OneShotDequeueWorkerBindingStatusV1" in contract
    assert "binding_record_fingerprint as v046_binding_record_fingerprint" in contract
    assert "status_fingerprint as v046_status_fingerprint" in contract
    assert "recognized_v046_binding_count" in contract
...
def test_v047_release_closure_does_not_add_effect_configuration() -> None:
    assert not ROOT.joinpath("compose.execution-smoke.override.yaml").exists()

    production_files = (
        "services/atlas-core/app/worker_binding_activation_preflight/contract.py",
        "services/atlas-core/app/worker_binding_activation_preflight/service.py",
        "services/atlas-core/app/worker_binding_activation_preflight/store.py",
        "services/atlas-core/app/routes/worker_binding_activation_preflight.py",
    )
...
def test_v047_default_off_api_ui_agent_and_execution_worker_isolation() -> None:
    service = _read(
        "services/atlas-core/app/worker_binding_activation_preflight/service.py"
    )
    store = _read("services/atlas-core/app/worker_binding_activation_preflight/store.py")
    route = _read("services/atlas-core/app/routes/worker_binding_activation_preflight.py")
    main = _read("services/atlas-core/app/main.py")
    mission_control_structural = _read(
        "services/mission-control/src/security/"
        "workerBindingActivationPreflightStructural.test.ts"
    )
    ...
    assert "UPDATE worker_binding_activation" not in store
    assert "DELETE FROM worker_binding_activation" not in store
    assert "worker_binding_activation_preflight_service" not in main
    assert "create_worker_binding_activation_preflight_service" not in main

    assert '@router.post(\n    "/{candidate_record_id}/worker-binding-activation-preflights"' in route
    assert '@router.get(\n    "/{candidate_record_id}/worker-binding-activation-preflights"' in route
    assert (
        '@router.get(\n    "/{candidate_record_id}/worker-binding-activation-preflights/'
        '{preflight_id}"'
        in route
    )
    ...
    assert "v0.47 worker binding activation preflight Mission Control boundary" in (
        mission_control_structural
    )
    assert r"atlas\.(post|put|patch|delete)" in mission_control_structural
    assert "worker-binding-activation-preflights|WorkerBindingActivationPreflight" in (
        mission_control_structural
    )
    ...
    markers = (
        ...
        "WorkerBindingActivationPreflight",
        "worker-binding-activation-preflight",
