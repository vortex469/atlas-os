"""Append-only durable store for v0.41 live enqueue admission evidence."""
...
MAX_RECORDS_PER_OPERATOR = MAX_COLLECTION_RECORDS


class LiveEnqueueAdmissionStoreError(RuntimeError):
...
class LiveEnqueueAdmissionStore:
    ...
    def _initialize(self) -> None:
        ...
        try:
            with self._connect() as connection:
                ...
                connection.execute(
                    """
                    CREATE TABLE IF NOT EXISTS live_enqueue_admissions (
                        operator_id TEXT NOT NULL,
                        admission_id TEXT NOT NULL,
                        candidate_record_id TEXT NOT NULL,
                        worker_intake_admission_valid_until TEXT NOT NULL,
                        idempotency_key_fingerprint TEXT NOT NULL,
                        worker_intake_admission_fingerprint TEXT NOT NULL,
                        worker_intake_admission_status_fingerprint TEXT NOT NULL,
                        ...
                        live_enqueue_admission_decision_fingerprint TEXT NOT NULL,
            ...
            raise LiveEnqueueAdmissionStoreError("unavailable") from error
    ...
    def resolve_idempotency(
        ...
        *,
    ...
    ) -> LiveEnqueueAdmissionV1 | None:
        try:
            with self._connect() as connection:
                row = connection.execute(
                    """SELECT * FROM live_enqueue_admissions
        ...
        if row is None:
            return None
        if (
            row["worker_intake_admission_valid_until"]
            != worker_intake_admission_valid_until
        ):
            raise LiveEnqueueAdmissionStoreError("conflict")
    ...
    def append(
        ...
        *,
        ...
        reservation: LiveEnqueueAdmissionSubjectReservationV1,
        audit_evidence: LiveEnqueueAdmissionAuditEvidenceV1,
        worker_intake_admission_valid_until: str,
    ) -> tuple[LiveEnqueueAdmissionV1, bool]:
        record_json = record.model_dump_json()
        reservation_json = reservation.model_dump_json()
        audit_json = audit_evidence.model_dump_json()
        if (
            max(len(value.encode()) for value in (record_json, reservation_json, audit_json))
            > self.max_model_bytes
        ):
            raise LiveEnqueueAdmissionStoreError("record_too_large")
        try:
            with self._connect() as connection:
                connection.execute("BEGIN IMMEDIATE")
                try:
                    rows = connection.execute(
                        """SELECT * FROM live_enqueue_admissions WHERE operator_id = ?
                    ...
                    if rows:
                        if len(rows) != 1 or not self._is_idempotent_row(
                    ...
                    count = connection.execute(
                        """SELECT COUNT(*) FROM live_enqueue_admissions
                        WHERE operator_id = ?""",
                        (record.operator_id,),
                    ).fetchone()[0]
                    if count >= self.max_records_per_operator:
                        raise LiveEnqueueAdmissionStoreError("quota_exceeded")
                    link = record.linkage
                    connection.execute(
                        """INSERT INTO live_enqueue_admissions (
                        ...
                        (
                            ...
                            link.worker_intake_admission_status_fingerprint.value,
                            ...
                            link.live_enqueue_admission_decision_fingerprint.value,
                    ...
                    raise
        except LiveEnqueueAdmissionStoreError:
            raise
        except sqlite3.IntegrityError as error:
            raise LiveEnqueueAdmissionStoreError("conflict") from error
        except sqlite3.Error as error:
            raise LiveEnqueueAdmissionStoreError("unavailable") from error

    def get(self, *, operator_id: str, admission_id: str) -> LiveEnqueueAdmissionV1:
        try:
            with self._connect() as connection:
                row = connection.execute(
                    """SELECT * FROM live_enqueue_admissions
            ...
            raise LiveEnqueueAdmissionStoreError("unavailable") from error
        if row is None:
            raise LiveEnqueueAdmissionStoreError("not_found")
        return self._decode(row, operator_id=operator_id)

    def list_owned(
        self, *, operator_id: str, candidate_record_id: str
    ) -> tuple[LiveEnqueueAdmissionV1, ...]:
        try:
            with self._connect() as connection:
                rows = connection.execute(
                    """SELECT * FROM live_enqueue_admissions
        ...
        if len(rows) > self.max_records_per_operator:
            raise LiveEnqueueAdmissionStoreError("unavailable")
        return tuple(self._decode(row, operator_id=operator_id) for row in rows)

    @staticmethod
    def _is_idempotent_row(
        row: sqlite3.Row,
        record: LiveEnqueueAdmissionV1,
        reservation: LiveEnqueueAdmissionSubjectReservationV1,
    ) -> bool:
    ...
    def _is_exact(
        ...
        return (
            ...
            == record.admission_id
            == reservation.admission_id
            and row["worker_intake_admission_fingerprint"]
            == link.worker_intake_admission_fingerprint.value
            == reservation.worker_intake_admission_fingerprint.value
            and row["worker_intake_admission_status_fingerprint"]
            == link.worker_intake_admission_status_fingerprint.value
            ...
            and row["live_enqueue_admission_decision_fingerprint"]
            == link.live_enqueue_admission_decision_fingerprint.value
            == reservation.live_enqueue_admission_decision_fingerprint.value
    ...
    def _decode(
        ...
        try:
            ...
            record = LiveEnqueueAdmissionV1.model_validate_json(row["record_json"])
            reservation = LiveEnqueueAdmissionSubjectReservationV1.model_validate_json(
                row["reservation_json"]
            )
            audit = LiveEnqueueAdmissionAuditEvidenceV1.model_validate_json(
                row["audit_json"]
            )
            if (
                operator_id != row["operator_id"]
                or row["worker_intake_admission_valid_until"] < record.valid_until
                ...
                or audit.event != "live_enqueue_admission_recorded"
            ...
            raise LiveEnqueueAdmissionStoreError("store_corrupt") from error
