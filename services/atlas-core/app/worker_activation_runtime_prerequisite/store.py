...
_SCHEMA = (
    """CREATE TABLE reservations (
        ...
        UNIQUE (operator_id, candidate_record_id, admission_id))""",
...
class WorkerActivationRuntimePrerequisiteStoreError(RuntimeError):
...
class WorkerActivationRuntimePrerequisiteStore:
    def __init__(
        self,
        ...
        *,
        ...
        for value, maximum in (
            ...
            if type(value) is not int or not 0 <= value <= maximum:
                raise WorkerActivationRuntimePrerequisiteStoreError("invalid_request")
        ...
        with self._connect() as connection:
            ...
            if actual:
                if actual != self._schema or marker != 53:
                    raise WorkerActivationRuntimePrerequisiteStoreError("store_corrupt")
            else:
                if marker != 0:
                    raise WorkerActivationRuntimePrerequisiteStoreError("store_corrupt")
                for statement in _SCHEMA:
                    connection.execute(statement)
                connection.execute("PRAGMA application_id=53")
    ...
    def _connect(self) -> Iterator[sqlite3.Connection]:
        ...
        try:
            ...
            if self.max_database_bytes < size:
                raise WorkerActivationRuntimePrerequisiteStoreError("quota_exceeded")
            connection.execute(
            ...
            raise WorkerActivationRuntimePrerequisiteStoreError(code) from None
        finally:
    ...
    def _check_integrity(self, connection):
        try:
            if (
                ...
                != self._schema
                or connection.execute("PRAGMA application_id").fetchone()[0] != 53
            ...
            for row in connection.execute("SELECT * FROM reservations"):
                reservation = self._decode(
                    c.WorkerActivationRuntimePrerequisiteSubjectReservationV1, row[6]
                )
                if row[:6] != (
            ...
            for subject, record_json, audit_json in connection.execute(
                ...
                record = self._decode(
                    c.WorkerActivationRuntimePrerequisiteV1, record_json
                )
                audit = self._decode(
                    c.WorkerActivationRuntimePrerequisiteAuditEvidenceV1, audit_json
                )
    ...
    def _validate_record(record, reservation):
        create = c.build_create(
            ...
            receipt_status=record.controlled_worker_queue_claim_lease_acknowledgement_status,
        )
        if (
            record.operator_id != reservation.operator_id
            or record.candidate_record_id != reservation.candidate_record_id
            or record.admission_id != reservation.admission_id
    ...
    def _validate_audit(audit, reservation, record):
        if (
            ...
            != (record.prerequisite_record_fingerprint if record is not None else None)
        ):
    ...
    def _resolve(self, connection, operator_id, idem, request):
        ...
        if row is None:
            return None
        if row[1] != request:
            raise WorkerActivationRuntimePrerequisiteStoreError("idempotency_conflict")
        evidence = connection.execute(
            "SELECT record_json FROM evidence WHERE subject=?", (row[0],)
        ).fetchone()
        if evidence is None:
            raise WorkerActivationRuntimePrerequisiteStoreError("append_indeterminate")
        return self._decode(c.WorkerActivationRuntimePrerequisiteV1, evidence[0])
    ...
    def append(
        ...
        *,
        ...
        try:
            reservation = c.WorkerActivationRuntimePrerequisiteSubjectReservationV1.model_validate(
                reservation
            )
            ...
            with self._connect() as connection:
                ...
                record = c.WorkerActivationRuntimePrerequisiteV1.model_validate(
                    prepare()
                )
                self._validate_record(record, reservation)
        ...
        except WorkerActivationRuntimePrerequisiteStoreError:
    ...
    def _audit(reservation, correlation, record):
        raw = {
            ...
            "worker_activation_runtime_prerequisite_recorded": record is not None,
        }
        return c._signed(
    ...
    def _bounded(self, model):
        if len(model.model_dump_json().encode()) > self.max_model_bytes:
            raise WorkerActivationRuntimePrerequisiteStoreError("record_too_large")

    def get(self, *, operator_id, candidate_record_id, prerequisite_id):
        with self._connect() as connection:
            for (payload,) in connection.execute(
                ...
                record = self._decode(c.WorkerActivationRuntimePrerequisiteV1, payload)
                if record.prerequisite_id == prerequisite_id:
                    return record
        raise WorkerActivationRuntimePrerequisiteStoreError("evidence_not_found")

    def list_owned(self, *, operator_id, candidate_record_id):
        with self._connect() as connection:
            return tuple(
                self._decode(c.WorkerActivationRuntimePrerequisiteV1, row[0])
