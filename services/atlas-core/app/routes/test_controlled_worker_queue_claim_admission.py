"""P3 API locks for v0.49 controlled worker queue claim admission."""
...
from app.controlled_worker_queue_claim_admission.test_service_store import _service
from app.operator_auth.models import (
    INSTALLATION_CONTROLLED_WORKER_QUEUE_CLAIM_ADMISSION_READ,
    INSTALLATION_CONTROLLED_WORKER_QUEUE_CLAIM_ADMISSION_RECORD,
...
from app.routes.controlled_worker_queue_claim_admission import router
...
def _application(
    ...
    *,
    ...
    admission_service, _store, evidence_reader, activation, _status, create = _service(
    ...
    if service_installed:
        application.state.controlled_worker_queue_claim_admission_service = (
            admission_service
        )
    collection = (
        f"/api/v1/installation/candidate-records/{activation.candidate_record_id}"
...
def _headers(session, key: str = "controlled-worker-claim-route-key-1"):
    return {
        "Origin": ORIGIN,
...
def test_auth_csrf_success_readback_and_exact_no_replay(tmp_path: Path) -> None:
    client, session, activation, create, evidence_reader, url, sessions = _application(
        tmp_path
    )
    ...
    assert body["controlled_worker_queue_claim_admission_recorded"] is True
    assert body["record"]["candidate_record_id"] == activation.candidate_record_id
    assert body["record"]["admission_state"] == "readiness_gated"
    assert body["record"]["eligibility"] == (
        "controlled_worker_queue_claim_admission_recorded"
    )
    assert body["record"]["worker_binding_activation_evidence"][
        "activation_evidence_id"
    ] == payload["activation_evidence_id"]
    ...
    foreign = sessions.create(
        OperatorCredential(
            ...
            password_hash="unused",
            permissions=(INSTALLATION_CONTROLLED_WORKER_QUEUE_CLAIM_ADMISSION_READ,),
        )
    )
...
def test_dedicated_permissions_default_off_and_redaction(tmp_path: Path) -> None:
    ...
    create_client, create_session, _, _create, _, url, _ = _application(
        tmp_path / "create",
        permissions=(INSTALLATION_CONTROLLED_WORKER_QUEUE_CLAIM_ADMISSION_RECORD,),
    )
    assert create_client.get(url, cookies=_cookies(create_session)).status_code == 403

    disabled, disabled_session, _, create, evidence_reader, url, _ = _application(
    ...
    assert "controlled-worker-claim-route-key-1" not in json.dumps(unavailable.json())
...
def test_strict_body_query_method_rate_and_idempotency_bounds(tmp_path: Path) -> None:
    ...
    assert client.post(
        url,
        json={**payload, "endpoint": "https://worker.invalid/queue"},
        cookies=cookies,
        headers=headers,
    ).status_code == 422
    assert client.post(
    ...
    assert limited_client.post(
        ...
        headers=_headers(limited_session, "controlled-worker-claim-first-key"),
    ).status_code == 201
    limited = limited_client.post(
        limited_url,
        json=limited_create.model_dump(mode="json"),
        cookies=_cookies(limited_session),
        headers=_headers(limited_session, "controlled-worker-claim-second-key"),
    )
...
def test_permissions_openapi_method_and_authority_isolation() -> None:
    assert INSTALLATION_CONTROLLED_WORKER_QUEUE_CLAIM_ADMISSION_RECORD == (
        "installation.execution.controlled_worker_queue_claim_admission.record"
    )
    assert INSTALLATION_CONTROLLED_WORKER_QUEUE_CLAIM_ADMISSION_READ == (
        "installation.execution.controlled_worker_queue_claim_admission.read"
    )
    assert {
        INSTALLATION_CONTROLLED_WORKER_QUEUE_CLAIM_ADMISSION_RECORD,
        INSTALLATION_CONTROLLED_WORKER_QUEUE_CLAIM_ADMISSION_READ,
    ...
    assert not any(
        ...
        for path in paths
        if "controlled-worker-queue-claim-admissions" in path
        for segment in path.split("controlled-worker-queue-claim-admissions", 1)[1]
        .strip("/")
        .split("/")
        if segment
    )
...
def test_route_has_no_runtime_effect_imports_or_broad_calls() -> None:
    path = Path(__file__).with_name("controlled_worker_queue_claim_admission.py")
    ...
    allowed_contract_import = "app.controlled_worker_queue_claim_admission.contract"
    assert not [
        name
        for name in imports
...
def test_production_startup_does_not_construct_queue_claim_admission() -> None:
    source = Path(__file__).parents[1].joinpath("main.py").read_text(
        encoding="utf-8"
    )
    assert "controlled_worker_queue_claim_admission_service" not in source
    assert "create_controlled_worker_queue_claim_admission_service" not in source
