...
router = APIRouter(
    ...
    tags=["Worker Activation Runtime Definition"],
)
_ERRORS = {
    code: {"model": WorkerActivationRuntimeDefinitionErrorV1}
...
async def _body(request: Request) -> WorkerActivationRuntimeDefinitionCreateV1:
    ...
    if request.headers.getlist("transfer-encoding") and (
        lengths or request.headers.getlist("transfer-encoding") != ["chunked"]
    ):
...
def _correlation(value: str) -> str:
    return fingerprint(
        "correlation",
        value if type(value) is str and 0 < len(value) <= 128 else "redacted",
    )
...
def _error(code: str, status: int, correlation: str) -> JSONResponse:
    return JSONResponse(
        ...
        content=jsonable_encoder(
            WorkerActivationRuntimeDefinitionErrorV1(
                error_code=code, correlation_fingerprint=_correlation(correlation)
            )
        ),
    )
...
def _auth_error(error: HTTPException, correlation: str) -> JSONResponse:
    return _error(
        ...
        error.status_code if error.status_code in {401, 503} else 403,
        correlation,
    )
...
def _service(request: Request):
    service = getattr(
        request.app.state, "worker_activation_runtime_definition_service", None
    )
    if service is None:
...
def _validated(result, operator: str, candidate: str):
    ...
    if (
        value.record.operator_id != operator
        or value.record.candidate_record_id != candidate
    ):
...
def _service_error(result: WorkerActivationRuntimeDefinitionErrorV1) -> JSONResponse:
    status = {
        ...
        "store_corrupt": 503,
        "unavailable": 503,
        "installation_capability_unsupported": 503,
...
@router.post(
    "/{candidate_record_id}/worker-activation-runtime-definitions",
    response_model=WorkerActivationRuntimeDefinitionResultV1,
    status_code=201,
    responses=_ERRORS,
    summary="Record bounded worker activation runtime definition evidence",
    openapi_extra={
        "parameters": [
            {
        ...
        "requestBody": {
            ...
            "content": {
                "application/json": {
                    "schema": WorkerActivationRuntimeDefinitionCreateV1.model_json_schema()
                }
            },
        },
    },
)
async def create_worker_activation_runtime_definition(
    request: Request,
    response: Response,
    ...
    try:
        ...
        if isinstance(result, WorkerActivationRuntimeDefinitionResultV1):
            ...
            if (
                ...
                or record.definition_id != payload.definition_id
                or record.valid_until != payload.valid_until
                or record.definition.definition_fingerprint
    ...
    # The endpoint is a fail-closed evidence boundary: unexpected parser,
    # dependency, and storage failures must never escape as authority.
...
@router.get(
    "/{candidate_record_id}/worker-activation-runtime-definitions",
    response_model=WorkerActivationRuntimeDefinitionCollectionV1,
    responses=_ERRORS,
    summary="List owned worker activation runtime definition evidence",
)
async def list_worker_activation_runtime_definitions(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
    ...
    try:
        ...
        if isinstance(result, WorkerActivationRuntimeDefinitionErrorV1):
            result = WorkerActivationRuntimeDefinitionErrorV1.model_validate(
                _plain(result)
            )
        else:
            result = WorkerActivationRuntimeDefinitionCollectionV1.model_validate(
                _plain(result)
            )
            if (
        ...
        return _auth_error(error, correlation)
    # Keep unexpected failures redacted and unavailable at this boundary.
    except Exception:  # noqa: BLE001
        return _error("unavailable", 503, correlation)
    return (
        _service_error(result)
        if isinstance(result, WorkerActivationRuntimeDefinitionErrorV1)
        else result
    )
...
@router.get(
    "/{candidate_record_id}/worker-activation-runtime-definitions/{definition_id}",
    response_model=WorkerActivationRuntimeDefinitionResultV1,
    responses=_ERRORS,
    summary="Read owned worker activation runtime definition evidence",
)
async def get_worker_activation_runtime_definition(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
    ...
    if (
        ...
        or not _canonical_uuid(candidate_record_id, 4)
        or not _canonical_uuid(definition_id, 5)
    ):
        return _error("invalid_request", 422, correlation)
    try:
        principal = _read_permission(request)
        ...
        if (
            isinstance(result, WorkerActivationRuntimeDefinitionResultV1)
            and result.record.definition_id != definition_id
        ):
            raise HTTPException(404)
    except HTTPException as error:
        if error.status_code == 404:
