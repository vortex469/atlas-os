"""P3 API locks for v0.44 controlled dequeue admission evidence."""
...
from app.controlled_dequeue_admission.test_service_store import _service
...
from app.routes.controlled_dequeue_admission import router
...
def _application(
    ...
    *,
    ...
    admission_service, _store, reader, receipt, _status, create = _service(
        tmp_path / "service",
        evidence=evidence,
        second=second,
    ...
    if service_installed:
        application.state.controlled_dequeue_admission_service = admission_service
    collection = (
        f"/api/v1/installation/candidate-records/{receipt.candidate_record_id}"
...
def _headers(session, key: str = "controlled-dequeue-route-key-1"):
    return {
        "Origin": ORIGIN,
...
def test_auth_permission_csrf_origin_success_readback_and_exact_retry(
    tmp_path: Path,
) -> None:
    client, session, _, receipt, create, reader, url, _ = _application(tmp_path)
    ...
    assert body["controlled_dequeue_admission_recorded"] is True
    assert body["record"]["candidate_record_id"] == receipt.candidate_record_id
    assert body["record"]["queue_observation_receipt"]["receipt_id"] == receipt.receipt_id
    assert body["record"]["queue_observation_receipt"]["v042_enqueue"][
        "enqueue_id"
    ] == receipt.v042_enqueue.enqueue_id
    assert body["record"]["eligibility"] == "eligible_for_later_dequeue_consideration"
    ...
    fetched = client.get(
        f"{url}/{body['record']['admission_id']}", cookies=_cookies(session)
    )
...
def test_dedicated_create_and_read_permissions_are_independent(tmp_path: Path) -> None:
    read_client, read_session, _, _, create, _, url, _ = _application(
        tmp_path / "read",
        permissions=(INSTALLATION_CONTROLLED_DEQUEUE_ADMISSION_READ,),
    )
    ...
    create_client, create_session, _, _, create, _, url, _ = _application(
        tmp_path / "create",
        permissions=(INSTALLATION_CONTROLLED_DEQUEUE_ADMISSION_RECORD,),
    )
...
def test_strict_body_query_method_rate_and_idempotency_bounds(tmp_path: Path) -> None:
    ...
    assert client.post(
        url + "?dequeue=true", json=payload, cookies=cookies, headers=headers
    ).status_code == 422
    ...
    assert client.post(
        ...
        json=payload,
        cookies=cookies,
        headers=_headers(session, "controlled-dequeue-rate-key"),
    ).status_code == 201
    ...
    limited = limited_client.post(
        ...
        headers=_headers(limited_session, "controlled-dequeue-first-key"),
    )
    ...
    limited = limited_client.post(
        ...
        json=limited_create.model_dump(mode="json"),
        cookies=_cookies(limited_session),
        headers=_headers(limited_session, "controlled-dequeue-next-key"),
    )
...
def test_default_off_stale_mismatch_redaction_and_foreign_read(tmp_path: Path) -> None:
    ...
    client, session, _, receipt, create, _, url, sessions = _application(
        tmp_path / "mismatch"
    )
    bad = create.model_dump(mode="json")
    bad["inherited_limits_fingerprint"]["value"] = "f" * 64
    mismatch = client.post(
        url, json=bad, cookies=_cookies(session), headers=_headers(session)
    )
    assert mismatch.status_code == 409
    assert mismatch.json()["error"]["error_code"] == "inherited_limits_mismatch"
    rendered = json.dumps(mismatch.json()).lower()
    assert "controlled-dequeue-route-key-1" not in rendered
    ...
    made = client.post(
        ...
        json=create.model_dump(mode="json"),
        cookies=_cookies(session),
        headers=_headers(session, "controlled-dequeue-read-key"),
    )
    assert made.status_code == 201
    ...
    assert client.get(f"{url}/{admission_id}", cookies=_cookies(session)).status_code == 200
    ...
    hidden = url.replace(
        receipt.candidate_record_id, "bf819229-618a-44f5-a14e-4c0f5878ea14"
    )
    assert client.get(f"{hidden}/{admission_id}", cookies=_cookies(session)).status_code == 404


def test_permissions_openapi_and_no_sibling_effect_routes() -> None:
    assert INSTALLATION_CONTROLLED_DEQUEUE_ADMISSION_RECORD == (
        "installation.execution.controlled_dequeue_admission.record"
    )
    assert INSTALLATION_CONTROLLED_DEQUEUE_ADMISSION_READ == (
    ...
    controlled_paths = {
        ...
        if "controlled-dequeue-admissions" in path
    }
    prohibited_segments = {
        "poll",
        "claim",
    ...
    assert not any(
        ...
        for segment in path.split("controlled-dequeue-admissions", 1)[1]
        .strip("/")
        .split("/")
        if segment
    )
...
def test_route_has_no_runtime_effect_dependencies_or_authority_calls() -> None:
    path = Path(__file__).with_name("controlled_dequeue_admission.py")
...
def test_production_startup_does_not_construct_controlled_dequeue_service() -> None:
    source = Path(__file__).parents[1].joinpath("main.py").read_text()
    assert "controlled_dequeue_admission_service" not in source
    assert "create_controlled_dequeue_admission_service" not in source
