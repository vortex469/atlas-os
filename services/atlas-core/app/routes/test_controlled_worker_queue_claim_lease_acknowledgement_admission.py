"""P3 API locks for v0.51 queue claim/lease/ack admission evidence."""
...
def _application(
    ...
    *,
    ...
    admission_service, _store, reader, prerequisite, _status, create = _service(
    ...
    if service_installed:
        application.state.controlled_worker_queue_claim_lease_acknowledgement_admission_service = (
            admission_service
        )
    collection = (
    ...
    return ASGITestClient(application), session, prerequisite, create, reader, collection, sessions
...
def test_auth_csrf_success_readback_and_exact_no_replay(tmp_path: Path) -> None:
    ...
    assert (
        body["controlled_worker_queue_claim_lease_acknowledgement_admission_recorded"]
        is True
    )
    assert body["record"]["candidate_record_id"] == prerequisite.candidate_record_id
    assert body["record"]["admission_state"] == "recorded"
    assert body["record"]["eligibility"] == (
        "controlled_worker_queue_claim_lease_acknowledgement_admission_recorded"
    )
    assert body["record"]["prerequisite_id"] == payload["prerequisite_id"]
    assert body["record"]["prerequisite_record_fingerprint"] == (
        payload["prerequisite_record_fingerprint"]
    )
    assert body["record"]["prerequisite_status_fingerprint"] == (
        payload["prerequisite_status_fingerprint"]
    )
    assert body["record"]["v049_admission_record_fingerprint"] == (
        payload["v049_admission_record_fingerprint"]
    )
    assert body["record"]["v049_admission_status_fingerprint"] == (
        payload["v049_admission_status_fingerprint"]
    )
...
def test_dedicated_permissions_default_off_and_redaction(tmp_path: Path) -> None:
    read_client, read_session, _, create, _, url, _ = _application(
        tmp_path / "read",
        permissions=(INSTALLATION_CONTROLLED_WORKER_QUEUE_CLAIM_LEASE_ACK_ADMISSION_READ,),
    )
...
def test_permissions_openapi_method_and_authority_isolation() -> None:
    assert INSTALLATION_CONTROLLED_WORKER_QUEUE_CLAIM_LEASE_ACK_ADMISSION_RECORD == (
        "installation.execution."
        "controlled_worker_queue_claim_lease_acknowledgement_admission.record"
    )
    assert INSTALLATION_CONTROLLED_WORKER_QUEUE_CLAIM_LEASE_ACK_ADMISSION_READ == (
        "installation.execution."
    ...
    assert not any(
        ...
        for path in paths
        if "controlled-worker-queue-claim-lease-acknowledgement-admissions" in path
        for segment in path.split(
            "controlled-worker-queue-claim-lease-acknowledgement-admissions", 1
        )[1]
        .strip("/")
        .split("/")
        if segment
...
def test_production_startup_does_not_construct_v051_admission_service() -> None:
