"""P3 API locks for v0.53 worker activation runtime prerequisite evidence."""
...
from app.operator_auth.models import (
    INSTALLATION_WORKER_ACTIVATION_RUNTIME_PREREQUISITE_EVALUATE,
    INSTALLATION_WORKER_ACTIVATION_RUNTIME_PREREQUISITE_READ,
...
def _application(
    ...
    *,
    ...
    if service_installed:
        application.state.worker_activation_runtime_prerequisite_service = (
            admission_service
        )
    collection = (
        f"/api/v1/installation/candidate-records/{prerequisite.candidate_record_id}"
        "/worker-activation-runtime-prerequisites"
    )
    return (
...
def test_auth_csrf_success_readback_and_exact_no_replay(tmp_path: Path, facts) -> None:
    ...
    body = made.json()
    assert body["worker_activation_runtime_prerequisite_recorded"] is True
    ...
    foreign = sessions.create(
        OperatorCredential(
            ...
            password_hash="unused",
            permissions=(INSTALLATION_WORKER_ACTIVATION_RUNTIME_PREREQUISITE_READ,),
        )
    )
...
def test_dedicated_permissions_default_off_and_redaction(tmp_path: Path, facts) -> None:
    ...
    create_client, create_session, _, _create, _, url, _ = _application(
        tmp_path / "create",
        facts,
        permissions=(INSTALLATION_WORKER_ACTIVATION_RUNTIME_PREREQUISITE_EVALUATE,),
    )
...
def test_permissions_openapi_method_and_authority_isolation() -> None:
    ...
    assert INSTALLATION_WORKER_ACTIVATION_RUNTIME_PREREQUISITE_EVALUATE == PERMISSION
    assert INSTALLATION_WORKER_ACTIVATION_RUNTIME_PREREQUISITE_EVALUATE == (
        "installation.execution.worker_activation_runtime_prerequisite.evaluate"
    )
    assert INSTALLATION_WORKER_ACTIVATION_RUNTIME_PREREQUISITE_READ == (
        "installation.execution.worker_activation_runtime_prerequisite.read"
    )
    assert {
        INSTALLATION_WORKER_ACTIVATION_RUNTIME_PREREQUISITE_EVALUATE,
        INSTALLATION_WORKER_ACTIVATION_RUNTIME_PREREQUISITE_READ,
    ...
    for parameter in paths[item]["get"]["parameters"]:
        examples = {
            ...
            "prerequisite_id": "a70ea6f4-18ba-57f3-867e-f5eae39bfb2d",
        }
    ...
    assert not any(
        ...
        for path in paths
        if "worker-activation-runtime-prerequisites" in path
        for segment in path.split("worker-activation-runtime-prerequisites", 1)[1]
        .strip("/")
        .split("/")
        if segment
    )
...
def test_route_has_no_runtime_effect_imports_or_broad_calls() -> None:
    path = Path(__file__).with_name("worker_activation_runtime_prerequisite.py")
    ...
    allowed_contract_import = "app.worker_activation_runtime_prerequisite.contract"
    assert not [
        name
        for name in imports
...
def test_production_startup_does_not_construct_v053_admission_service() -> None:
    source = Path(__file__).parents[1].joinpath("main.py").read_text(encoding="utf-8")
    assert "worker_activation_runtime_prerequisite_service" not in source
    assert "create_worker_activation_runtime_prerequisite_service" not in source
...
def test_lineage_isolation_staleness_and_permanent_reservation(
    ...
    service = client._app.state.worker_activation_runtime_prerequisite_service
    ...
    assert made.status_code == 201
    item_id = made.json()["record"]["prerequisite_id"]
...
def test_bounded_input_and_redacted_untrusted_service_outputs(
    ...
    assert made.status_code == 201
    item = f"{url}/{made.json()['record']['prerequisite_id']}"
    real = service.get(
        authenticated_operator_id=made.json()["record"]["operator_id"],
        permission_verified=True,
        prerequisite_id=made.json()["record"]["prerequisite_id"],
...
def test_chunked_body_stops_at_bound_without_content_length() -> None:
    ...
    from app.routes.worker_activation_runtime_prerequisite import _body
    from app.worker_activation_runtime_prerequisite.contract import (
        MAX_CREATE_BYTES,
    )
...
def test_malformed_requests_never_read_or_reserve(tmp_path, facts, mutation):
    from app.worker_activation_runtime_prerequisite.test_service_store import counts
...
def test_reparse_every_service_output_including_constructed_models(
    ...
    result = service.create(
        ...
        permission_verified=True,
        candidate_record_id=facts.candidate_record_id,
        idempotency_key="v053-output-validation",
        correlation_id="test",
    )
    assert isinstance(result, c.WorkerActivationRuntimePrerequisiteResultV1)
    item = f"{url}/{result.record.prerequisite_id}"
    if target == "result":
...
def test_corrupted_nested_prerequisite_cannot_create_evidence(tmp_path, facts):
    ...
    assert response.status_code == 422
    assert counts(
        client._app.state.worker_activation_runtime_prerequisite_service._store
    ) == (0, 0, 0)


def test_exact_registered_surface_and_zero_agent_worker_consumers():
    collection = "/installation/candidate-records/{candidate_record_id}/worker-activation-runtime-prerequisites"
    assert {
        (route.path, method) for route in router.routes for method in route.methods
    } == {
        (collection, "GET"),
        (collection, "POST"),
        (collection + "/{prerequisite_id}", "GET"),
    }
    ...
    for directory in ("atlas-agent", "atlas-execution-worker"):
        for path in (root / "services" / directory).rglob("*.py"):
            ...
            assert "worker_activation_runtime_prerequisite" not in source, path
            assert "worker-activation-runtime-prerequisites" not in source, path
...
def test_valid_service_response_must_match_exact_request(tmp_path, facts, monkeypatch):
    ...
    result = service.create(
        ...
        permission_verified=True,
        candidate_record_id=facts.candidate_record_id,
        idempotency_key="v053-response-linkage",
        correlation_id="test",
    )
...
def test_unexpected_dependency_http_errors_are_redacted(tmp_path, facts, monkeypatch):
    ...
    monkeypatch.setattr(
        client._app.state.worker_activation_runtime_prerequisite_service, "create", fail
    )
    response = client.post(
        url,
        json=create.model_dump(mode="json"),
