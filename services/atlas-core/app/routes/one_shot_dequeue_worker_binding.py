"""Guarded Core API for v0.46 one-shot dequeue worker binding evidence."""
...
router = APIRouter(
    ...
    tags=["One-Shot Dequeue Worker Bindings"],
)

_ERRORS = {
    code: {"model": OneShotDequeueWorkerBindingResultV1}
    for code in (401, 403, 404, 409, 413, 415, 422, 429, 503)
}


async def _body(request: Request) -> OneShotDequeueWorkerBindingCreateV1:
...
def _service(request: Request):
    value = getattr(request.app.state, "one_shot_dequeue_worker_binding_service", None)
    if value is None:
...
def _correlation(value: str):
    ...
    return opaque_fingerprint(
        "atlas:one-shot-dequeue-worker-binding-correlation:v1", safe
    )
...
def _service_response(result: OneShotDequeueWorkerBindingResultV1) -> JSONResponse:
    ...
    status_code = {
        ...
        "v045_dequeue_not_active": 409,
        "v045_dequeue_not_recorded": 409,
        "v045_dequeue_not_successful": 409,
        "v040_worker_intake_not_active": 409,
        "v040_worker_intake_not_recorded": 409,
        "linkage_mismatch": 409,
        "worker_subject_mismatch": 409,
        "queue_item_reference_mismatch": 409,
...
@router.post(
    "/{candidate_record_id}/one-shot-dequeue-worker-bindings",
    response_model=OneShotDequeueWorkerBindingResultV1,
    status_code=201,
    responses=_ERRORS,
    summary="Record bounded one-shot dequeue worker binding evidence",
    openapi_extra={
        "parameters": [
            {
        ...
        "requestBody": {
            ...
            "content": {
                "application/json": {
                    "schema": OneShotDequeueWorkerBindingCreateV1.model_json_schema()
                }
            },
        },
    },
)
async def create_one_shot_dequeue_worker_binding(
    request: Request,
    response: Response,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> OneShotDequeueWorkerBindingResultV1 | JSONResponse:
...
@router.get(
    "/{candidate_record_id}/one-shot-dequeue-worker-bindings",
    response_model=OneShotDequeueWorkerBindingCollectionV1,
    responses=_ERRORS,
    summary="List owned one-shot dequeue worker binding evidence",
)
async def list_one_shot_dequeue_worker_bindings(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> OneShotDequeueWorkerBindingCollectionV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if (
...
@router.get(
    "/{candidate_record_id}/one-shot-dequeue-worker-bindings/{binding_id}",
    response_model=OneShotDequeueWorkerBindingResultV1,
    responses=_ERRORS,
    summary="Read owned one-shot dequeue worker binding evidence",
)
async def get_one_shot_dequeue_worker_binding(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
    binding_id: Annotated[str, WithJsonSchema({"type": "string", "pattern": _UUID5})],
) -> OneShotDequeueWorkerBindingResultV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if (
        request.query_params
        or await request.body()
        or not _canonical_uuid(candidate_record_id, version=4)
        or not _canonical_uuid(binding_id, version=5)
    ):
        return _json_error("invalid_request", 422, correlation_id)
    try:
        principal = _read_permission(request)
...
def reject_one_shot_dequeue_worker_binding_collection_method() -> None:
...
def reject_one_shot_dequeue_worker_binding_item_method() -> None:
