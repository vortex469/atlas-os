"""Guarded Core API for v0.48 worker binding activation evidence."""
...
router = APIRouter(
    ...
    tags=["Worker Binding Activation Evidence"],
)

_ERRORS = {
    code: {"model": WorkerBindingActivationEvidenceResultV1}
    for code in (401, 403, 404, 409, 413, 415, 422, 429, 503)
}


async def _body(request: Request) -> WorkerBindingActivationEvidenceCreateV1:
...
def _service(request: Request):
    value = getattr(
        request.app.state, "worker_binding_activation_evidence_service", None
    )
    if value is None:
...
def _service_response(result: WorkerBindingActivationEvidenceResultV1) -> JSONResponse:
    ...
    status_code = {
        ...
        "v047_preflight_not_active": 409,
        "v047_preflight_not_recorded": 409,
...
@router.post(
    ...
    response_model=WorkerBindingActivationEvidenceResultV1,
    status_code=201,
    responses=_ERRORS,
    summary="Record bounded worker binding activation evidence",
    openapi_extra={
        "parameters": [
            {
        ...
        "requestBody": {
            ...
            "content": {
                "application/json": {
                    "schema": (
                        WorkerBindingActivationEvidenceCreateV1.model_json_schema()
                    )
                }
            },
        },
...
async def create_worker_binding_activation_evidence(
    request: Request,
    response: Response,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> WorkerBindingActivationEvidenceResultV1 | JSONResponse:
...
@router.get(
    "/{candidate_record_id}/worker-binding-activation-evidence",
    response_model=WorkerBindingActivationEvidenceCollectionV1,
    responses=_ERRORS,
    summary="List owned worker binding activation evidence",
)
async def list_worker_binding_activation_evidence(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> WorkerBindingActivationEvidenceCollectionV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if (
...
@router.get(
    "/{candidate_record_id}/worker-binding-activation-evidence/{activation_evidence_id}",
    response_model=WorkerBindingActivationEvidenceResultV1,
    responses=_ERRORS,
    summary="Read owned worker binding activation evidence",
)
async def get_worker_binding_activation_evidence(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
    activation_evidence_id: Annotated[
    ...
    if (
        ...
        or await request.body()
        or not _canonical_uuid(candidate_record_id, version=4)
        or not _canonical_uuid(activation_evidence_id, version=5)
    ):
        return _json_error("invalid_request", 422, correlation_id)
    try:
        principal = _read_permission(request)
