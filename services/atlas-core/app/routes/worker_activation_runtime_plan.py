"""Guarded Core API for v0.55 worker activation runtime plan evidence."""
...
from app.worker_activation_runtime_plan.contract import (
    MAX_CREATE_BYTES,
    WorkerActivationRuntimePlanCollectionV1,
    WorkerActivationRuntimePlanCreateV1,
    WorkerActivationRuntimePlanRedactedErrorV1,
    WorkerActivationRuntimePlanResultV1,
    _plain,
    fingerprint,
...
router = APIRouter(
    ...
    tags=["Worker Activation Runtime Plan"],
)

_ERRORS = {
    code: {"model": WorkerActivationRuntimePlanRedactedErrorV1}
...
def _service(request: Request):
    value = getattr(
        request.app.state,
        "worker_activation_runtime_plan_service",
        None,
    )
    if value is None:
...
def _validated_result(result, operator_id: str, candidate_record_id: str):
    if isinstance(result, WorkerActivationRuntimePlanRedactedErrorV1):
        return WorkerActivationRuntimePlanRedactedErrorV1.model_validate(
            _plain(result)
        )
    result = WorkerActivationRuntimePlanResultV1.model_validate(_plain(result))
    if (
...
def _service_response(
    result: WorkerActivationRuntimePlanRedactedErrorV1,
) -> JSONResponse:
...
@router.post(
    "/{candidate_record_id}/worker-activation-runtime-plans",
    response_model=WorkerActivationRuntimePlanResultV1,
    status_code=201,
    responses=_ERRORS,
    summary="Record bounded worker activation runtime plan evidence",
    openapi_extra={
        "parameters": [
            {
        ...
        "requestBody": {
            ...
            "content": {
                "application/json": {
                    "schema": (
                        WorkerActivationRuntimePlanCreateV1.model_json_schema()
                    )
                }
            },
        },
...
async def create_worker_activation_runtime_plan(
    request: Request,
    response: Response,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> WorkerActivationRuntimePlanResultV1 | JSONResponse:
    ...
    try:
        ...
        if isinstance(result, WorkerActivationRuntimePlanResultV1):
            record = result.record
            receipt = record.worker_activation_runtime_admission
            status = record.worker_activation_runtime_admission_status
            if (
                record.idempotency_key_fingerprint
                != idempotency_key_fingerprint(principal.operator_id, key)
                or record.runtime_admission_id != payload.runtime_admission_id
                or record.valid_until != payload.valid_until
                or receipt.runtime_admission_record_fingerprint
                != payload.runtime_admission_record_fingerprint
                or status.status_fingerprint != payload.status_fingerprint
            ):
                raise ValueError("unexpected plan response")
...
@router.get(
    "/{candidate_record_id}/worker-activation-runtime-plans",
    response_model=WorkerActivationRuntimePlanCollectionV1,
    responses=_ERRORS,
    summary="List owned worker activation runtime plan evidence",
)
async def list_worker_activation_runtime_plans(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> WorkerActivationRuntimePlanCollectionV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if request.query_params or not _canonical_uuid(candidate_record_id, version=4):
        return _json_error("invalid_request", 422, correlation_id)
    try:
        ...
        if isinstance(result, WorkerActivationRuntimePlanRedactedErrorV1):
            result = WorkerActivationRuntimePlanRedactedErrorV1.model_validate(
                _plain(result)
            )
        else:
            result = WorkerActivationRuntimePlanCollectionV1.model_validate(
                _plain(result)
            )
            if (
...
@router.get(
    "/{candidate_record_id}/worker-activation-runtime-plans/{runtime_plan_id}",
    response_model=WorkerActivationRuntimePlanResultV1,
    responses=_ERRORS,
    summary="Read owned worker activation runtime plan evidence",
)
async def get_worker_activation_runtime_plan(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
    runtime_plan_id: Annotated[
    ...
    if (
        ...
        or not _canonical_uuid(runtime_plan_id, version=5)
    ):
        return _json_error("invalid_request", 422, correlation_id)
    try:
        principal = _read_permission(request)
        ...
        result = _validated_result(result, principal.operator_id, candidate_record_id)
        if (
            isinstance(result, WorkerActivationRuntimePlanResultV1)
            and result.record.runtime_plan_id != runtime_plan_id
        ):
            raise HTTPException(404)
    except HTTPException as error:
        if error.status_code == 404:
