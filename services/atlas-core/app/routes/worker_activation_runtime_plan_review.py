"""Guarded Core API for v0.56 worker activation runtime plan review evidence."""
...
from app.worker_activation_runtime_plan_review.contract import (
    MAX_CREATE_BYTES,
    WorkerActivationRuntimePlanReviewCollectionV1,
    WorkerActivationRuntimePlanReviewCreateV1,
    WorkerActivationRuntimePlanReviewRedactedErrorV1,
    WorkerActivationRuntimePlanReviewResultV1,
    _plain,
    fingerprint,
...
router = APIRouter(
    ...
    tags=["Worker Activation Runtime Plan Review"],
)

_ERRORS = {
    code: {"model": WorkerActivationRuntimePlanReviewRedactedErrorV1}
...
def _service(request: Request):
    value = getattr(
        request.app.state,
        "worker_activation_runtime_plan_review_service",
        None,
    )
    if value is None:
...
def _validated_result(result, operator_id: str, candidate_record_id: str):
    if isinstance(result, WorkerActivationRuntimePlanReviewRedactedErrorV1):
        return WorkerActivationRuntimePlanReviewRedactedErrorV1.model_validate(
            _plain(result)
        )
    result = WorkerActivationRuntimePlanReviewResultV1.model_validate(_plain(result))
    if (
...
def _service_response(
    result: WorkerActivationRuntimePlanReviewRedactedErrorV1,
) -> JSONResponse:
...
@router.post(
    "/{candidate_record_id}/worker-activation-runtime-plan-reviews",
    response_model=WorkerActivationRuntimePlanReviewResultV1,
    status_code=201,
    responses=_ERRORS,
    summary="Record bounded worker activation runtime plan review evidence",
    openapi_extra={
        "parameters": [
            {
        ...
        "requestBody": {
            ...
            "content": {
                "application/json": {
                    "schema": (
                        WorkerActivationRuntimePlanReviewCreateV1.model_json_schema()
                    )
                }
            },
        },
...
async def create_worker_activation_runtime_plan_review(
    request: Request,
    response: Response,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> WorkerActivationRuntimePlanReviewResultV1 | JSONResponse:
    ...
    try:
        ...
        if isinstance(result, WorkerActivationRuntimePlanReviewResultV1):
            record = result.record
            receipt = record.worker_activation_runtime_plan
            status = record.worker_activation_runtime_plan_status
            if (
                record.idempotency_key_fingerprint
                != idempotency_key_fingerprint(principal.operator_id, key)
                or record.runtime_plan_id != payload.runtime_plan_id
                or record.valid_until != payload.valid_until
                or receipt.runtime_plan_record_fingerprint
                != payload.runtime_plan_record_fingerprint
                or status.status_fingerprint != payload.status_fingerprint
            ):
                raise ValueError("unexpected plan review response")
...
@router.get(
    "/{candidate_record_id}/worker-activation-runtime-plan-reviews",
    response_model=WorkerActivationRuntimePlanReviewCollectionV1,
    responses=_ERRORS,
    summary="List owned worker activation runtime plan review evidence",
)
async def list_worker_activation_runtime_plan_reviews(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> WorkerActivationRuntimePlanReviewCollectionV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if request.query_params or not _canonical_uuid(candidate_record_id, version=4):
        return _json_error("invalid_request", 422, correlation_id)
    try:
        ...
        if isinstance(result, WorkerActivationRuntimePlanReviewRedactedErrorV1):
            result = WorkerActivationRuntimePlanReviewRedactedErrorV1.model_validate(
                _plain(result)
            )
        else:
            result = WorkerActivationRuntimePlanReviewCollectionV1.model_validate(
                _plain(result)
            )
            if (
...
@router.get(
    "/{candidate_record_id}/worker-activation-runtime-plan-reviews/{runtime_plan_review_id}",
    response_model=WorkerActivationRuntimePlanReviewResultV1,
    responses=_ERRORS,
    summary="Read owned worker activation runtime plan review evidence",
)
async def get_worker_activation_runtime_plan_review(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
    runtime_plan_review_id: Annotated[
    ...
    if (
        ...
        or not _canonical_uuid(runtime_plan_review_id, version=5)
    ):
        return _json_error("invalid_request", 422, correlation_id)
    try:
        principal = _read_permission(request)
        ...
        result = _validated_result(result, principal.operator_id, candidate_record_id)
        if (
            isinstance(result, WorkerActivationRuntimePlanReviewResultV1)
            and result.record.runtime_plan_review_id != runtime_plan_review_id
        ):
            raise HTTPException(404)
    except HTTPException as error:
        if error.status_code == 404:
