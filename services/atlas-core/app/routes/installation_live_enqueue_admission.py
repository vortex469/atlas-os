"""Guarded Core API for v0.41 live enqueue admission evidence."""
...
_read_permission = require_operator_permission(INSTALLATION_LIVE_ENQUEUE_ADMISSION_READ)

router = APIRouter(
    prefix="/installation/candidate-records",
    tags=["Live Enqueue Admissions"],
)

_ERRORS = {
    code: {"model": LiveEnqueueAdmissionResultV1}
...
async def _body(request: Request) -> LiveEnqueueAdmissionCreateV1:
    ...
    try:
        ...
        return LiveEnqueueAdmissionCreateV1.model_validate(decoded)
    except (
        UnicodeDecodeError,
...
def _service(request: Request):
    value = getattr(request.app.state, "live_enqueue_admission_service", None)
    if value is None:
...
def _error_result(error_code: str, correlation_id: str) -> LiveEnqueueAdmissionResultV1:
...
def _service_response(result: LiveEnqueueAdmissionResultV1) -> JSONResponse:
    ...
    status_code = {
        ...
        "queue_item_reference_invalid": 409,
...
@router.post(
    "/{candidate_record_id}/live-enqueue-admissions",
    response_model=LiveEnqueueAdmissionResultV1,
    status_code=201,
    responses=_ERRORS,
    summary="Record live enqueue admission evidence",
    openapi_extra={
        "parameters": [
            {
        ...
        "requestBody": {
            ...
            "content": {
                "application/json": {
                    "schema": LiveEnqueueAdmissionCreateV1.model_json_schema()
                }
            },
        },
    },
)
async def create_live_enqueue_admission(
    request: Request,
    response: Response,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> LiveEnqueueAdmissionResultV1 | JSONResponse:
...
@router.get(
    "/{candidate_record_id}/live-enqueue-admissions",
    response_model=LiveEnqueueAdmissionCollectionV1,
    responses=_ERRORS,
    summary="List owned live enqueue admission evidence",
)
async def list_live_enqueue_admissions(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> LiveEnqueueAdmissionCollectionV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if (
...
@router.get(
    "/{candidate_record_id}/live-enqueue-admissions/{admission_id}",
    response_model=LiveEnqueueAdmissionResultV1,
    responses=_ERRORS,
    summary="Read owned live enqueue admission evidence",
)
async def get_live_enqueue_admission(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
    ...
    if (
        ...
        or await request.body()
        or re.fullmatch(_UUID4, candidate_record_id, re.ASCII) is None
        or re.fullmatch(_UUID5, admission_id, re.ASCII) is None
    ):
        return _json_error("invalid_request", 422, correlation_id)
    try:
        principal = _read_permission(request)
...
def reject_live_enqueue_admission_collection_method() -> None:
...
def reject_live_enqueue_admission_item_method() -> None:
