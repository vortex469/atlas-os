"""Guarded Core API for v0.50 queue claim/lease/ack prerequisite evidence."""
...
router = APIRouter(
    ...
    tags=["Controlled Worker Queue Claim Lease Acknowledgement Prerequisite"],
)

_ERRORS = {
    code: {
        "model": ControlledWorkerQueueClaimLeaseAcknowledgementPrerequisiteResultV1
    }
...
def _correlation(value: str):
    ...
    return opaque_fingerprint(
        "atlas:controlled-worker-queue-claim-lease-ack-prerequisite-correlation:v1",
        safe,
    )
...
def _service_response(
    result: ControlledWorkerQueueClaimLeaseAcknowledgementPrerequisiteResultV1,
) -> JSONResponse:
    ...
    status_code = {
        ...
        "v049_admission_not_active": 409,
        "v049_admission_not_recorded": 409,
...
@router.post(
    "/{candidate_record_id}/controlled-worker-queue-claim-lease-acknowledgement-prerequisites",
    response_model=ControlledWorkerQueueClaimLeaseAcknowledgementPrerequisiteResultV1,
    status_code=201,
    responses=_ERRORS,
    summary="Record bounded queue claim/lease/ack prerequisite evidence",
    openapi_extra={
        "parameters": [
            {
        ...
        "requestBody": {
            ...
            "content": {
                "application/json": {
                    "schema": (
                        ControlledWorkerQueueClaimLeaseAcknowledgementPrerequisiteCreateV1.model_json_schema()
                    )
                }
            },
        },
...
async def create_controlled_worker_queue_claim_lease_acknowledgement_prerequisite(
    request: Request,
    response: Response,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> ControlledWorkerQueueClaimLeaseAcknowledgementPrerequisiteResultV1 | JSONResponse:
...
@router.get(
    "/{candidate_record_id}/controlled-worker-queue-claim-lease-acknowledgement-prerequisites",
    response_model=ControlledWorkerQueueClaimLeaseAcknowledgementPrerequisiteCollectionV1,
    responses=_ERRORS,
    summary="List owned queue claim/lease/ack prerequisite evidence",
)
async def list_controlled_worker_queue_claim_lease_acknowledgement_prerequisites(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
...
@router.get(
    "/{candidate_record_id}/controlled-worker-queue-claim-lease-acknowledgement-prerequisites/{prerequisite_id}",
    response_model=ControlledWorkerQueueClaimLeaseAcknowledgementPrerequisiteResultV1,
    responses=_ERRORS,
    summary="Read owned queue claim/lease/ack prerequisite evidence",
)
async def get_controlled_worker_queue_claim_lease_acknowledgement_prerequisite(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
