"""Guarded Core API for v0.47 worker binding activation preflight evidence."""
...
router = APIRouter(
    ...
    tags=["Worker Binding Activation Preflights"],
)

_ERRORS = {
    code: {"model": WorkerBindingActivationPreflightResultV1}
    for code in (401, 403, 404, 409, 413, 415, 422, 429, 503)
}


async def _body(request: Request) -> WorkerBindingActivationPreflightCreateV1:
...
def _service(request: Request):
    value = getattr(
        request.app.state, "worker_binding_activation_preflight_service", None
    )
    if value is None:
...
def _service_response(result: WorkerBindingActivationPreflightResultV1) -> JSONResponse:
    ...
    status_code = {
        ...
        "v046_binding_not_active": 409,
        "v046_binding_not_recorded": 409,
...
@router.post(
    ...
    response_model=WorkerBindingActivationPreflightResultV1,
    status_code=201,
    responses=_ERRORS,
    summary="Record bounded worker binding activation preflight evidence",
    openapi_extra={
        "parameters": [
            {
        ...
        "requestBody": {
            ...
            "content": {
                "application/json": {
                    "schema": (
                        WorkerBindingActivationPreflightCreateV1.model_json_schema()
                    )
                }
            },
        },
...
async def create_worker_binding_activation_preflight(
    request: Request,
    response: Response,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> WorkerBindingActivationPreflightResultV1 | JSONResponse:
...
@router.get(
    "/{candidate_record_id}/worker-binding-activation-preflights",
    response_model=WorkerBindingActivationPreflightCollectionV1,
    responses=_ERRORS,
    summary="List owned worker binding activation preflight evidence",
)
async def list_worker_binding_activation_preflights(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> WorkerBindingActivationPreflightCollectionV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if (
...
@router.get(
    "/{candidate_record_id}/worker-binding-activation-preflights/{preflight_id}",
    response_model=WorkerBindingActivationPreflightResultV1,
    responses=_ERRORS,
    summary="Read owned worker binding activation preflight evidence",
)
async def get_worker_binding_activation_preflight(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
    preflight_id: Annotated[str, WithJsonSchema({"type": "string", "pattern": _UUID5})],
) -> WorkerBindingActivationPreflightResultV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if (
        request.query_params
        or await request.body()
        or not _canonical_uuid(candidate_record_id, version=4)
        or not _canonical_uuid(preflight_id, version=5)
    ):
        return _json_error("invalid_request", 422, correlation_id)
    try:
        principal = _read_permission(request)
