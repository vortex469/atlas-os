"""P3 API locks for v0.57 worker activation runtime interface prerequisite evidence."""
...
from app.operator_auth.models import (
    INSTALLATION_WORKER_ACTIVATION_RUNTIME_INTERFACE_PREREQUISITE_EVALUATE,
    INSTALLATION_WORKER_ACTIVATION_RUNTIME_INTERFACE_PREREQUISITE_READ,
    ...
    *,
    ...
    def read_owned(**kwargs):
        if (
            ...
            or kwargs["runtime_plan_review_id"] != facts.create.runtime_plan_review_id
        ):
            reader.calls += 1
            return None
    ...
    if service_installed:
        application.state.worker_activation_runtime_interface_prerequisite_service = (
            review_service
        )
    collection = (
        f"/api/v1/installation/candidate-records/{prerequisite.candidate_record_id}"
        "/worker-activation-runtime-interface-prerequisites"
    )
    return (
...
def _headers(session, key: str = "v057-api-plan-review-key-1"):
    return {
        "Origin": ORIGIN,
...
def test_auth_csrf_success_readback_and_exact_no_replay(tmp_path: Path, facts) -> None:
    ...
    from app.worker_activation_runtime_interface_prerequisite.contract import (
        SUCCESS_BLOCKERS,
        ClosedAuthorityV1,
    )

    assert body["record"]["blockers"] == list(SUCCESS_BLOCKERS)
    assert body["record"]["inventory"] == [
        entry.model_dump(mode="json") for entry in p1.c.SUCCESS_INVENTORY
    ]
    assert body["record"]["worker_activation_runtime_plan_review"] == (
        facts.worker_activation_runtime_plan_review.model_dump(mode="json")
    )
    assert body["record"]["worker_activation_runtime_plan_review_status"] == (
        facts.worker_activation_runtime_plan_review_status.model_dump(mode="json")
    )
...
def test_dedicated_permissions_default_off_and_redaction(tmp_path: Path, facts) -> None:
    ...
    assert "v057-api-plan-review-key-1" not in json.dumps(unavailable.json())
...
def test_strict_body_query_method_rate_and_idempotency_bounds(
    ...
    assert (
        limited_client.post(
            ...
            headers=_headers(limited_session, "v057-api-review-first-key"),
        ).status_code
        == 201
    )
    limited = limited_client.post(
        ...
        json=limited_create.model_dump(mode="json"),
        cookies=_cookies(limited_session),
        headers=_headers(limited_session, "v057-api-review-second-key"),
    )
...
def test_permissions_openapi_method_and_authority_isolation() -> None:
    ...
    assert INSTALLATION_WORKER_ACTIVATION_RUNTIME_INTERFACE_PREREQUISITE_EVALUATE == (
        "installation.execution.worker_activation_runtime_interface_prerequisite.evaluate"
    )
    assert INSTALLATION_WORKER_ACTIVATION_RUNTIME_INTERFACE_PREREQUISITE_READ == (
        "installation.execution.worker_activation_runtime_interface_prerequisite.read"
    )
    assert {
        INSTALLATION_WORKER_ACTIVATION_RUNTIME_INTERFACE_PREREQUISITE_EVALUATE,
        INSTALLATION_WORKER_ACTIVATION_RUNTIME_INTERFACE_PREREQUISITE_READ,
    ...
    item = f"{collection}/{{runtime_interface_prerequisite_id}}"
    ...
    for parameter in paths[item]["get"]["parameters"]:
        examples = {
            ...
            "runtime_interface_prerequisite_id": "a70ea6f4-18ba-57f3-867e-f5eae39bfb2d",
        }
    ...
    assert not any(
        ...
        for path in paths
        if "worker-activation-runtime-interface-prerequisites" in path
        for segment in path.split(
            "worker-activation-runtime-interface-prerequisites", 1
        )[1]
        .strip("/")
        .split("/")
        if segment
...
def test_route_has_no_runtime_effect_imports_or_broad_calls() -> None:
    path = Path(__file__).with_name(
        "worker_activation_runtime_interface_prerequisite.py"
    )
    ...
    allowed_contract_import = (
        "app.worker_activation_runtime_interface_prerequisite.contract"
    )
    assert not [
        name
        for name in imports
...
def test_production_startup_does_not_construct_v057_review_service() -> None:
    source = Path(__file__).parents[1].joinpath("main.py").read_text(encoding="utf-8")
    assert "worker_activation_runtime_interface_prerequisite_service" not in source
    assert (
        "create_worker_activation_runtime_interface_prerequisite_service" not in source
    )
...
def test_lineage_isolation_staleness_and_permanent_reservation(
    ...
    assert made.status_code == 201
    item_id = made.json()["record"]["runtime_interface_prerequisite_id"]
...
def test_bounded_input_and_redacted_untrusted_service_outputs(
    ...
    assert made.status_code == 201
    item = f"{url}/{made.json()['record']['runtime_interface_prerequisite_id']}"
    real = service.get(
        authenticated_operator_id=made.json()["record"]["operator_id"],
        permission_verified=True,
        runtime_interface_prerequisite_id=made.json()["record"][
...
def test_chunked_body_stops_at_bound_without_content_length() -> None:
    ...
    from app.routes.worker_activation_runtime_interface_prerequisite import _body
    from app.worker_activation_runtime_interface_prerequisite.contract import (
        MAX_CREATE_BYTES,
    )
...
def test_reparse_every_service_output_including_constructed_models(
    ...
    result = service.create(
        ...
        permission_verified=True,
        candidate_record_id=facts.candidate_record_id,
        idempotency_key="v057-output-validation",
        correlation_id="test",
    )
    assert isinstance(result, c.WorkerActivationRuntimeInterfacePrerequisiteResultV1)
    item = f"{url}/{result.record.runtime_interface_prerequisite_id}"
    if target == "result":
...
def test_exact_registered_surface_and_zero_agent_worker_consumers():
    collection = "/installation/candidate-records/{candidate_record_id}/worker-activation-runtime-interface-prerequisites"
    assert {
        (route.path, method) for route in router.routes for method in route.methods
    } == {
        (collection, "GET"),
        (collection, "POST"),
        (collection + "/{runtime_interface_prerequisite_id}", "GET"),
    }
    ...
    for directory in ("atlas-agent", "atlas-execution-worker"):
        for path in (root / "services" / directory).rglob("*.py"):
            ...
            assert "worker_activation_runtime_interface_prerequisite" not in source, (
                path
            )
            assert "worker-activation-runtime-interface-prerequisites" not in source, (
                path
            )
...
def test_valid_service_response_must_match_exact_request(
    ...
    result = service.create(
        ...
        permission_verified=True,
        candidate_record_id=facts.candidate_record_id,
        idempotency_key="v057-response-linkage",
        correlation_id="test",
    )
    ...
    if field.endswith("fingerprint"):
        ...
        payload[field] = (
            ...
            if field == "runtime_plan_review_id"
    ...
    assert (
        client.post(
            ...
            json=payload,
            cookies=_cookies(session),
            headers=_headers(session, "v057-response-linkage"),
        ).status_code
        == 503
    )
...
def test_api_durable_lineage_restart_expiry_and_corrupt_readback(
    ...
    payload = c.build_create(
        receipt=record,
        receipt_status=c.v056.derive_status(record, evaluated_at=record.recorded_at),
    ...
    assert made.status_code == 201
    admitted = (
        c.WorkerActivationRuntimeInterfacePrerequisiteResultV1.model_validate_json(
            made.text
        )
    )
    assert (
    ...
    item = f"{url}/{admitted.record.runtime_interface_prerequisite_id}"
    assert (
...
def test_route_rejects_valid_foreign_service_results(tmp_path, facts, monkeypatch):
    ...
    for response in (
        ...
        client.get(
            f"{url}/{result.record.runtime_interface_prerequisite_id}",
            cookies=_cookies(foreign),
        ),
        client.post(
            url,
...
def test_oversized_service_envelopes_fail_closed(tmp_path, facts, monkeypatch, target):
    ...
    result = service.create(
        ...
        permission_verified=True,
        candidate_record_id=facts.candidate_record_id,
        idempotency_key="v057-envelope-bound",
        correlation_id="test",
    )
    assert isinstance(result, c.WorkerActivationRuntimeInterfacePrerequisiteResultV1)
    if target == "collection":
        ...
        method, path = "get", f"{url}/{result.record.runtime_interface_prerequisite_id}"
...
def test_inventory_cannot_be_removed_from_service_response(
    tmp_path, facts, monkeypatch
):
    from app.worker_activation_runtime_interface_prerequisite import contract as c

    client, session, _, _, _, url, _ = _application(tmp_path, facts)
    record = c.build_runtime_interface_prerequisite(
        facts, idempotency_key="v057-inventory-response"
    )
    ...
    bad = result.model_copy(
        update={"record": record.model_copy(update={"inventory": ()})}
    )
    service = client._app.state.worker_activation_runtime_interface_prerequisite_service
    monkeypatch.setattr(service, "get", lambda **kwargs: bad)
    response = client.get(
        f"{url}/{record.runtime_interface_prerequisite_id}", cookies=_cookies(session)
    )
    assert response.status_code == 503
