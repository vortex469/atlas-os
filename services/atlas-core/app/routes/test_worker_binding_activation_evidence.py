"""P3 API locks for v0.48 worker binding activation evidence."""
...
from app.operator_auth.models import (
    INSTALLATION_WORKER_BINDING_ACTIVATION_EVIDENCE_READ,
    INSTALLATION_WORKER_BINDING_ACTIVATION_EVIDENCE_RECORD,
...
from app.routes.worker_binding_activation_evidence import router
from app.testing import ASGITestClient
from app.worker_binding_activation_evidence.test_service_store import (
    _service,
)
...
def _application(
    ...
    *,
    ...
    evidence_service, _store, preflight_reader, preflight, _preflight_status, create = (
    ...
    if service_installed:
        application.state.worker_binding_activation_evidence_service = evidence_service
    collection = (
        f"/api/v1/installation/candidate-records/{preflight.candidate_record_id}"
...
def _headers(session, key: str = "worker-binding-evidence-route-key-1"):
    return {
        "Origin": ORIGIN,
...
def test_auth_csrf_success_readback_and_exact_no_replay(tmp_path: Path) -> None:
    client, session, preflight, create, preflight_reader, url, sessions = _application(
        tmp_path
    )
    ...
    assert body["worker_binding_activation_evidence_recorded"] is True
    assert body["record"]["candidate_record_id"] == preflight.candidate_record_id
    assert body["record"]["activation_evidence_state"] == "readiness_gated"
    assert body["record"]["eligibility"] == (
        "worker_binding_activation_evidence_recorded"
    )
    assert body["record"]["worker_binding_activation_preflight"]["preflight_id"] == (
    ...
    fetched = client.get(
        f"{url}/{body['record']['activation_evidence_id']}",
        cookies=_cookies(session),
    )
    ...
    foreign = sessions.create(
        OperatorCredential(
            ...
            password_hash="unused",
            permissions=(INSTALLATION_WORKER_BINDING_ACTIVATION_EVIDENCE_READ,),
        )
    )
...
def test_dedicated_permissions_default_off_and_redaction(tmp_path: Path) -> None:
    ...
    create_client, create_session, _, _create, _, url, _ = _application(
        tmp_path / "create",
        permissions=(INSTALLATION_WORKER_BINDING_ACTIVATION_EVIDENCE_RECORD,),
    )
    assert create_client.get(url, cookies=_cookies(create_session)).status_code == 403

    disabled, disabled_session, _, create, preflight_reader, url, _ = _application(
    ...
    assert "worker-binding-evidence-route-key-1" not in json.dumps(unavailable.json())
...
def test_strict_body_query_method_rate_and_idempotency_bounds(tmp_path: Path) -> None:
    ...
    assert limited_client.post(
        ...
        headers=_headers(limited_session, "worker-binding-evidence-first-key"),
    ).status_code == 201
    limited = limited_client.post(
        limited_url,
        json=limited_create.model_dump(mode="json"),
        cookies=_cookies(limited_session),
        headers=_headers(limited_session, "worker-binding-evidence-second-key"),
    )
...
def test_permissions_openapi_method_and_authority_isolation() -> None:
    assert INSTALLATION_WORKER_BINDING_ACTIVATION_EVIDENCE_RECORD == (
        "installation.execution.worker_binding_activation_evidence.record"
    )
    assert INSTALLATION_WORKER_BINDING_ACTIVATION_EVIDENCE_READ == (
        "installation.execution.worker_binding_activation_evidence.read"
    )
    assert {
        INSTALLATION_WORKER_BINDING_ACTIVATION_EVIDENCE_RECORD,
        INSTALLATION_WORKER_BINDING_ACTIVATION_EVIDENCE_READ,
    ...
    item = f"{collection}/{{activation_evidence_id}}"
    ...
    assert not any(
        ...
        for path in paths
        if "worker-binding-activation-evidence" in path
        for segment in path.split("worker-binding-activation-evidence", 1)[1]
        .strip("/")
        .split("/")
        if segment
    )
...
def test_route_has_no_runtime_effect_imports_or_broad_calls() -> None:
    path = Path(__file__).with_name("worker_binding_activation_evidence.py")
...
def test_production_startup_does_not_construct_worker_binding_evidence() -> None:
    source = Path(__file__).parents[1].joinpath("main.py").read_text(
        encoding="utf-8"
    )
    assert "worker_binding_activation_evidence_service" not in source
    assert "create_worker_binding_activation_evidence_service" not in source
