"""Guarded Core API for v0.44 controlled dequeue admission evidence."""
...
router = APIRouter(
    ...
    tags=["Controlled Dequeue Admissions"],
)

_ERRORS = {
    code: {"model": ControlledDequeueAdmissionResultV1}
    for code in (401, 403, 404, 409, 413, 415, 422, 429, 503)
}


async def _body(request: Request) -> ControlledDequeueAdmissionCreateV1:
...
def _service(request: Request):
    value = getattr(request.app.state, "controlled_dequeue_admission_service", None)
    if value is None:
...
def _service_response(result: ControlledDequeueAdmissionResultV1) -> JSONResponse:
...
@router.post(
    "/{candidate_record_id}/controlled-dequeue-admissions",
    response_model=ControlledDequeueAdmissionResultV1,
    status_code=201,
    responses=_ERRORS,
    summary="Record bounded controlled dequeue admission evidence",
    openapi_extra={
        "parameters": [
            {
        ...
        "requestBody": {
            ...
            "content": {
                "application/json": {
                    "schema": ControlledDequeueAdmissionCreateV1.model_json_schema()
                }
            },
        },
    },
)
async def create_controlled_dequeue_admission(
    request: Request,
    response: Response,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> ControlledDequeueAdmissionResultV1 | JSONResponse:
...
@router.get(
    "/{candidate_record_id}/controlled-dequeue-admissions",
    response_model=ControlledDequeueAdmissionCollectionV1,
    responses=_ERRORS,
    summary="List owned controlled dequeue admission evidence",
)
async def list_controlled_dequeue_admissions(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> ControlledDequeueAdmissionCollectionV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if (
...
@router.get(
    "/{candidate_record_id}/controlled-dequeue-admissions/{admission_id}",
    response_model=ControlledDequeueAdmissionResultV1,
    responses=_ERRORS,
    summary="Read owned controlled dequeue admission evidence",
)
async def get_controlled_dequeue_admission(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
...
def reject_controlled_dequeue_admission_collection_method() -> None:
...
def reject_controlled_dequeue_admission_item_method() -> None:
