"""Guarded Core API for v0.53 worker activation runtime prerequisite evidence."""
...
from app.worker_activation_runtime_prerequisite.contract import (
    MAX_CREATE_BYTES,
    WorkerActivationRuntimePrerequisiteCollectionV1,
    WorkerActivationRuntimePrerequisiteCreateV1,
    WorkerActivationRuntimePrerequisiteRedactedErrorV1,
    WorkerActivationRuntimePrerequisiteResultV1,
    _plain,
    fingerprint,
    parse_create_json,
)
...
router = APIRouter(
    ...
    tags=["Worker Activation Runtime Prerequisite"],
)

_ERRORS = {
    code: {"model": WorkerActivationRuntimePrerequisiteRedactedErrorV1}
...
def _service(request: Request):
    value = getattr(
        request.app.state,
        "worker_activation_runtime_prerequisite_service",
        None,
    )
    if value is None:
...
def _validated_result(result, operator_id: str, candidate_record_id: str):
    if isinstance(result, WorkerActivationRuntimePrerequisiteRedactedErrorV1):
        return WorkerActivationRuntimePrerequisiteRedactedErrorV1.model_validate(
            _plain(result)
        )
    result = WorkerActivationRuntimePrerequisiteResultV1.model_validate(_plain(result))
    if (
...
def _service_response(
    result: WorkerActivationRuntimePrerequisiteRedactedErrorV1,
) -> JSONResponse:
...
@router.post(
    "/{candidate_record_id}/worker-activation-runtime-prerequisites",
    response_model=WorkerActivationRuntimePrerequisiteResultV1,
    status_code=201,
    responses=_ERRORS,
    summary="Record bounded worker activation runtime prerequisite evidence",
    openapi_extra={
        "parameters": [
            {
        ...
        "requestBody": {
            ...
            "content": {
                "application/json": {
                    "schema": (
                        WorkerActivationRuntimePrerequisiteCreateV1.model_json_schema()
                    )
                }
            },
        },
...
async def create_worker_activation_runtime_prerequisite(
    request: Request,
    response: Response,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> WorkerActivationRuntimePrerequisiteResultV1 | JSONResponse:
    ...
    try:
        ...
        if isinstance(result, WorkerActivationRuntimePrerequisiteResultV1):
            record = result.record
            receipt = record.controlled_worker_queue_claim_lease_acknowledgement
            status = record.controlled_worker_queue_claim_lease_acknowledgement_status
            if (
                record.admission_id != payload.admission_id
                or record.valid_until != payload.valid_until
                or receipt.receipt_record_fingerprint
                != payload.receipt_record_fingerprint
                or status.status_fingerprint != payload.status_fingerprint
            ):
                raise ValueError("unexpected prerequisite response")
...
@router.get(
    "/{candidate_record_id}/worker-activation-runtime-prerequisites",
    response_model=WorkerActivationRuntimePrerequisiteCollectionV1,
    responses=_ERRORS,
    summary="List owned worker activation runtime prerequisite evidence",
)
async def list_worker_activation_runtime_prerequisites(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> WorkerActivationRuntimePrerequisiteCollectionV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if request.query_params or not _canonical_uuid(candidate_record_id, version=4):
        return _json_error("invalid_request", 422, correlation_id)
    try:
        ...
        if isinstance(result, WorkerActivationRuntimePrerequisiteRedactedErrorV1):
            result = WorkerActivationRuntimePrerequisiteRedactedErrorV1.model_validate(
                _plain(result)
            )
        else:
            result = WorkerActivationRuntimePrerequisiteCollectionV1.model_validate(
                _plain(result)
            )
            if (
...
@router.get(
    "/{candidate_record_id}/worker-activation-runtime-prerequisites/{prerequisite_id}",
    response_model=WorkerActivationRuntimePrerequisiteResultV1,
    responses=_ERRORS,
    summary="Read owned worker activation runtime prerequisite evidence",
)
async def get_worker_activation_runtime_prerequisite(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
    prerequisite_id: Annotated[
    ...
    if (
        ...
        or not _canonical_uuid(prerequisite_id, version=5)
    ):
        return _json_error("invalid_request", 422, correlation_id)
    try:
        principal = _read_permission(request)
        ...
        result = _validated_result(result, principal.operator_id, candidate_record_id)
        if (
            isinstance(result, WorkerActivationRuntimePrerequisiteResultV1)
            and result.record.prerequisite_id != prerequisite_id
        ):
            raise HTTPException(404)
    except HTTPException as error:
        if error.status_code == 404:
