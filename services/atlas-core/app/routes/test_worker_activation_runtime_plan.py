"""P3 API locks for v0.55 worker activation runtime plan evidence."""
...
from app.operator_auth.models import (
    INSTALLATION_WORKER_ACTIVATION_RUNTIME_PLAN_EVALUATE,
    INSTALLATION_WORKER_ACTIVATION_RUNTIME_PLAN_READ,
...
def _application(
    ...
    *,
    ...
    admission_service, _store, reader, _clock = setup(
    ...
    def read_owned(**kwargs):
        if (
            ...
            or kwargs["runtime_admission_id"] != facts.create.runtime_admission_id
        ):
            reader.calls += 1
            return None
    ...
    if service_installed:
        application.state.worker_activation_runtime_plan_service = (
            admission_service
        )
    collection = (
        f"/api/v1/installation/candidate-records/{prerequisite.candidate_record_id}"
        "/worker-activation-runtime-plans"
    )
    return (
...
def test_auth_csrf_success_readback_and_exact_no_replay(tmp_path: Path, facts) -> None:
    ...
    body = made.json()
    assert body["worker_activation_runtime_plan_recorded"] is True
    assert body["record"]["candidate_record_id"] == prerequisite.candidate_record_id
    from app.worker_activation_runtime_plan.contract import (
        SUCCESS_BLOCKERS,
        ClosedAuthorityV1,
    )
    ...
    fetched = client.get(
        f"{url}/{body['record']['runtime_plan_id']}",
        cookies=_cookies(session),
    )
    ...
    foreign = sessions.create(
        OperatorCredential(
            ...
            password_hash="unused",
            permissions=(INSTALLATION_WORKER_ACTIVATION_RUNTIME_PLAN_READ,),
        )
    )
    foreign_get = client.get(
        f"{url}/{body['record']['runtime_plan_id']}", cookies=_cookies(foreign)
    )
...
def test_dedicated_permissions_default_off_and_redaction(tmp_path: Path, facts) -> None:
    ...
    create_client, create_session, _, _create, _, url, _ = _application(
        tmp_path / "create",
        facts,
        permissions=(INSTALLATION_WORKER_ACTIVATION_RUNTIME_PLAN_EVALUATE,),
    )
...
def test_permissions_openapi_method_and_authority_isolation() -> None:
    ...
    assert INSTALLATION_WORKER_ACTIVATION_RUNTIME_PLAN_EVALUATE == PERMISSION
    assert INSTALLATION_WORKER_ACTIVATION_RUNTIME_PLAN_EVALUATE == (
        "installation.execution.worker_activation_runtime_plan.evaluate"
    )
    assert INSTALLATION_WORKER_ACTIVATION_RUNTIME_PLAN_READ == (
    ...
    item = f"{collection}/{{runtime_plan_id}}"
    ...
    for parameter in paths[item]["get"]["parameters"]:
        examples = {
            ...
            "runtime_plan_id": "a70ea6f4-18ba-57f3-867e-f5eae39bfb2d",
        }
    ...
    assert not any(
        ...
        for path in paths
        if "worker-activation-runtime-plans" in path
        for segment in path.split("worker-activation-runtime-plans", 1)[1]
        .strip("/")
        .split("/")
        if segment
    )
...
def test_route_has_no_runtime_effect_imports_or_broad_calls() -> None:
    path = Path(__file__).with_name("worker_activation_runtime_plan.py")
    ...
    allowed_contract_import = "app.worker_activation_runtime_plan.contract"
    assert not [
        name
        for name in imports
...
def test_production_startup_does_not_construct_v055_plan_service() -> None:
    source = Path(__file__).parents[1].joinpath("main.py").read_text(encoding="utf-8")
    assert "worker_activation_runtime_plan_service" not in source
    assert "create_worker_activation_runtime_plan_service" not in source
...
def test_lineage_isolation_staleness_and_permanent_reservation(
    ...
    client, session, admission, create, reader, url, sessions = _application(
        tmp_path, facts
    )
    cookies, headers = _cookies(session), _headers(session)
    payload = create.model_dump(mode="json")
    ...
    service = client._app.state.worker_activation_runtime_plan_service
    ...
    assert made.status_code == 201
    item_id = made.json()["record"]["runtime_plan_id"]
...
def test_bounded_input_and_redacted_untrusted_service_outputs(
    ...
    assert made.status_code == 201
    item = f"{url}/{made.json()['record']['runtime_plan_id']}"
    real = service.get(
        authenticated_operator_id=made.json()["record"]["operator_id"],
        permission_verified=True,
        runtime_plan_id=made.json()["record"]["runtime_plan_id"],
...
def test_chunked_body_stops_at_bound_without_content_length() -> None:
    ...
    from app.routes.worker_activation_runtime_plan import _body
    from app.worker_activation_runtime_plan.contract import (
        MAX_CREATE_BYTES,
    )
...
def test_reparse_every_service_output_including_constructed_models(
    ...
    result = service.create(
        ...
        permission_verified=True,
        candidate_record_id=facts.candidate_record_id,
        idempotency_key="v055-output-validation",
        correlation_id="test",
    )
    assert isinstance(result, c.WorkerActivationRuntimePlanResultV1)
    item = f"{url}/{result.record.runtime_plan_id}"
    if target == "result":
...
def test_corrupted_nested_prerequisite_cannot_create_evidence(tmp_path, facts):
    ...
    assert response.status_code == 422
    assert counts(
        client._app.state.worker_activation_runtime_plan_service._store
    ) == (0, 0, 0)


def test_exact_registered_surface_and_zero_agent_worker_consumers():
    collection = "/installation/candidate-records/{candidate_record_id}/worker-activation-runtime-plans"
    assert {
        (route.path, method) for route in router.routes for method in route.methods
    } == {
        (collection, "GET"),
        (collection, "POST"),
        (collection + "/{runtime_plan_id}", "GET"),
    }
    ...
    for directory in ("atlas-agent", "atlas-execution-worker"):
        for path in (root / "services" / directory).rglob("*.py"):
            ...
            assert "worker_activation_runtime_plan" not in source, path
            assert "worker-activation-runtime-plans" not in source, path
...
@pytest.mark.parametrize("field", [
    "runtime_admission_id", "valid_until", "runtime_admission_record_fingerprint", "status_fingerprint",
])
def test_valid_service_response_must_match_exact_request(tmp_path, facts, monkeypatch, field):
    ...
    result = service.create(
        ...
        permission_verified=True,
        candidate_record_id=facts.candidate_record_id,
        idempotency_key="v055-response-linkage",
        correlation_id="test",
    )
    ...
    if field.endswith("fingerprint"):
        ...
        payload[field] = (
            ...
            if field == "runtime_admission_id" else "2099-01-01T00:00:00Z"
        )
    assert (
        client.post(
            url, json=payload, cookies=_cookies(session),
            headers=_headers(session, "v055-response-linkage"),
        ).status_code
        == 503
    )
...
def test_unexpected_dependency_http_errors_are_redacted(tmp_path, facts, monkeypatch):
    ...
    monkeypatch.setattr(
        client._app.state.worker_activation_runtime_plan_service, "create", fail
    )
    response = client.post(
        url,
        json=create.model_dump(mode="json"),
...
def test_response_must_bind_exact_idempotency_key(tmp_path, facts, monkeypatch):
    ...
    monkeypatch.setattr(service, "create", lambda *args, **kwargs: result)
    response = client.post(
        url, json=create.model_dump(mode="json"),
...
@pytest.mark.parametrize(
    ...
    [
        ([("Content-Type", "application/json"), ("Content-Type", "application/json")], 415),
        ([("Content-Type", "application/json"), ("Content-Length", "-1")], 413),
        ([("Content-Type", "application/json"), ("Content-Length", "99999999999")], 413),
        ([("Content-Type", "application/json"), ("Content-Length", "1"), ("Content-Length", "1")], 413),
    ],
)
def test_ambiguous_headers_fail_before_read_or_reservation(
    ...
    client, session, _, create, reader, url, _ = _application(tmp_path, facts)
    response = client.post(
        url, content=create.model_dump_json(), cookies=_cookies(session),
        headers=[*_headers(session).items(), *extra_headers],
    )
    assert response.status_code == expected
    assert reader.calls == 0
    assert counts(client._app.state.worker_activation_runtime_plan_service._store) == (0, 0, 0)
...
def test_api_durable_lineage_restart_expiry_and_corrupt_readback(
    ...
    service._prerequisite_reader = WorkerActivationRuntimePlanPrerequisiteStoreReader(
        store=type(journal)(journal.database_path), clock=clock,
    )
    payload = c.build_create(
        receipt=record,
        receipt_status=c.v054.derive_status(record, evaluated_at=record.recorded_at),
    ).model_dump(mode="json")
    foreign = sessions.create(OperatorCredential(
        operator_id="foreign", password_hash="unused",
        permissions=(INSTALLATION_WORKER_ACTIVATION_RUNTIME_PLAN_EVALUATE,),
    ))
    denied = client.post(url, json=payload, cookies=_cookies(foreign), headers=_headers(foreign))
    assert denied.status_code == 404
    assert counts(service._store) == (0, 0, 0)
    made = client.post(url, json=payload, cookies=_cookies(session), headers=_headers(session))
    assert made.status_code == 201
    admitted = c.WorkerActivationRuntimePlanResultV1.model_validate_json(made.text)
    assert admitted.record.worker_activation_runtime_admission.model_dump_json() == record.model_dump_json()
    assert journal.database_path.read_bytes() == prior_bytes
    service._store = WorkerActivationRuntimePlanStore(service._store.database_path)
    service._clock.now += timedelta(seconds=31)
    clock.now += timedelta(seconds=31)

    def no_read(**kwargs):
        pytest.fail("historical API reads/duplicates must not read predecessor evidence")

    service._prerequisite_reader.read_owned = no_read
    duplicate = client.post(url, json=payload, cookies=_cookies(session), headers=_headers(session))
    assert duplicate.status_code == 201
    assert duplicate.json()["exact_duplicate"] is True
    assert duplicate.json()["status"]["lifecycle"] == "expired"
    assert duplicate.json()["record"] == made.json()["record"]
    assert counts(service._store) == (1, 1, 0)
    item = f"{url}/{admitted.record.runtime_plan_id}"
    assert client.get(item, cookies=_cookies(session)).json()["status"]["lifecycle"] == "expired"
    ...
    for response in (
        ...
        client.post(url, json=payload, cookies=_cookies(session), headers=_headers(session)),
    ):
        assert response.status_code == 503
...
def test_route_rejects_valid_foreign_service_results(tmp_path, facts, monkeypatch):
    ...
    for response in (
        ...
        client.get(f"{url}/{result.record.runtime_plan_id}", cookies=_cookies(foreign)),
...
def test_oversized_service_envelopes_fail_closed(tmp_path, facts, monkeypatch, target):
    ...
    result = service.create(
        create, authenticated_operator_id=facts.operator_id, permission_verified=True,
        candidate_record_id=facts.candidate_record_id,
        idempotency_key="v055-envelope-bound", correlation_id="test",
    )
    assert isinstance(result, c.WorkerActivationRuntimePlanResultV1)
    if target == "collection":
        ...
        method, path = "get", f"{url}/{result.record.runtime_plan_id}"
