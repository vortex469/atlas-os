"""Guarded Core API for v0.42 one-shot live enqueue evidence."""
...
_create_permission = require_operator_mutation(INSTALLATION_ONE_SHOT_LIVE_ENQUEUE_RECORD)
_read_permission = require_operator_permission(INSTALLATION_ONE_SHOT_LIVE_ENQUEUE_READ)

router = APIRouter(
    prefix="/installation/candidate-records",
    tags=["One-Shot Live Enqueues"],
)

_ERRORS = {
    code: {"model": OneShotLiveEnqueueResultV1}
...
async def _body(request: Request) -> OneShotLiveEnqueueCreateV1:
    ...
    try:
        ...
        return OneShotLiveEnqueueCreateV1.model_validate(decoded)
    except (
        UnicodeDecodeError,
...
def _service(request: Request):
    value = getattr(request.app.state, "one_shot_live_enqueue_service", None)
    if value is None:
...
def _error_result(error_code: str, correlation_id: str) -> OneShotLiveEnqueueResultV1:
...
def _service_response(result: OneShotLiveEnqueueResultV1) -> JSONResponse:
    ...
    status_code = {
        ...
        "live_enqueue_admission_not_active": 409,
        "live_enqueue_admission_not_recorded": 409,
        "worker_intake_admission_not_active": 409,
        "queue_reservation_not_active": 409,
        "worker_identity_ineligible": 409,
        "worker_intake_reference_ineligible": 409,
        "queue_intake_reference_ineligible": 409,
        "queue_item_reference_ineligible": 409,
...
@router.post(
    "/{candidate_record_id}/one-shot-live-enqueues",
    response_model=OneShotLiveEnqueueResultV1,
    status_code=201,
    responses=_ERRORS,
    summary="Record one inert reference-only queue item",
    openapi_extra={
        "parameters": [
            {
        ...
        "requestBody": {
            ...
            "content": {
                "application/json": {
                    "schema": OneShotLiveEnqueueCreateV1.model_json_schema()
                }
            },
        },
    },
)
async def create_one_shot_live_enqueue(
    request: Request,
    response: Response,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> OneShotLiveEnqueueResultV1 | JSONResponse:
...
@router.get(
    "/{candidate_record_id}/one-shot-live-enqueues",
    response_model=OneShotLiveEnqueueCollectionV1,
    responses=_ERRORS,
    summary="List owned one-shot live enqueue records",
)
async def list_one_shot_live_enqueues(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> OneShotLiveEnqueueCollectionV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if (
...
@router.get(
    "/{candidate_record_id}/one-shot-live-enqueues/{enqueue_id}",
    response_model=OneShotLiveEnqueueResultV1,
    responses=_ERRORS,
    summary="Read owned one-shot live enqueue record",
)
async def get_one_shot_live_enqueue(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
    enqueue_id: Annotated[str, WithJsonSchema({"type": "string", "pattern": _UUID5})],
) -> OneShotLiveEnqueueResultV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if (
        request.query_params
        or await request.body()
        or re.fullmatch(_UUID4, candidate_record_id, re.ASCII) is None
        or re.fullmatch(_UUID5, enqueue_id, re.ASCII) is None
    ):
        return _json_error("invalid_request", 422, correlation_id)
    try:
        principal = _read_permission(request)
...
def reject_one_shot_live_enqueue_collection_method() -> None:
...
def reject_one_shot_live_enqueue_item_method() -> None:
