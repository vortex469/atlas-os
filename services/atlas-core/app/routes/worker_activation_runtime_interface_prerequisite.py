"""Guarded Core API for v0.57 worker activation runtime interface prerequisite evidence."""
...
router = APIRouter(
    ...
    tags=["Worker Activation Runtime Interface Prerequisite"],
)

_ERRORS = {
    code: {"model": WorkerActivationRuntimeInterfacePrerequisiteRedactedErrorV1}
...
def _service(request: Request):
    value = getattr(
        request.app.state,
        "worker_activation_runtime_interface_prerequisite_service",
        None,
    )
    if value is None:
...
def _validated_result(result, operator_id: str, candidate_record_id: str):
    if isinstance(result, WorkerActivationRuntimeInterfacePrerequisiteRedactedErrorV1):
        return (
            WorkerActivationRuntimeInterfacePrerequisiteRedactedErrorV1.model_validate(
                _plain(result)
            )
        )
    result = WorkerActivationRuntimeInterfacePrerequisiteResultV1.model_validate(
        _plain(result)
    )
    if (
...
def _service_response(
    result: WorkerActivationRuntimeInterfacePrerequisiteRedactedErrorV1,
) -> JSONResponse:
...
@router.post(
    "/{candidate_record_id}/worker-activation-runtime-interface-prerequisites",
    response_model=WorkerActivationRuntimeInterfacePrerequisiteResultV1,
    status_code=201,
    responses=_ERRORS,
    summary="Record bounded worker activation runtime interface prerequisite evidence",
    openapi_extra={
        "parameters": [
            {
        ...
        "requestBody": {
            ...
            "content": {
                "application/json": {
                    "schema": (
                        WorkerActivationRuntimeInterfacePrerequisiteCreateV1.model_json_schema()
                    )
                }
            },
        },
...
async def create_worker_activation_runtime_interface_prerequisite(
    request: Request,
    response: Response,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> WorkerActivationRuntimeInterfacePrerequisiteResultV1 | JSONResponse:
    ...
    try:
        ...
        if isinstance(result, WorkerActivationRuntimeInterfacePrerequisiteResultV1):
            ...
            status = record.worker_activation_runtime_plan_review_status
            if (
                record.idempotency_key_fingerprint
                != idempotency_key_fingerprint(principal.operator_id, key)
                or record.runtime_plan_review_id != payload.runtime_plan_review_id
                or record.valid_until != payload.valid_until
                or receipt.runtime_plan_review_record_fingerprint
                != payload.runtime_plan_review_record_fingerprint
                or status.status_fingerprint != payload.status_fingerprint
            ):
                raise ValueError("unexpected interface prerequisite response")
...
@router.get(
    "/{candidate_record_id}/worker-activation-runtime-interface-prerequisites",
    response_model=WorkerActivationRuntimeInterfacePrerequisiteCollectionV1,
    responses=_ERRORS,
    summary="List owned worker activation runtime interface prerequisite evidence",
)
...
) -> WorkerActivationRuntimeInterfacePrerequisiteCollectionV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if request.query_params or not _canonical_uuid(candidate_record_id, version=4):
        return _json_error("invalid_request", 422, correlation_id)
    try:
        ...
        if isinstance(
            ...
            result = WorkerActivationRuntimeInterfacePrerequisiteRedactedErrorV1.model_validate(
                _plain(result)
            )
        else:
            result = (
                WorkerActivationRuntimeInterfacePrerequisiteCollectionV1.model_validate(
                    _plain(result)
                )
            )
            if (
...
@router.get(
    "/{candidate_record_id}/worker-activation-runtime-interface-prerequisites/{runtime_interface_prerequisite_id}",
    response_model=WorkerActivationRuntimeInterfacePrerequisiteResultV1,
    responses=_ERRORS,
    summary="Read owned worker activation runtime interface prerequisite evidence",
)
async def get_worker_activation_runtime_interface_prerequisite(
    ...
    runtime_interface_prerequisite_id: Annotated[
    ...
    if (
        ...
        or not _canonical_uuid(runtime_interface_prerequisite_id, version=5)
    ):
        return _json_error("invalid_request", 422, correlation_id)
    try:
        principal = _read_permission(request)
        ...
        result = _validated_result(result, principal.operator_id, candidate_record_id)
        if (
            isinstance(result, WorkerActivationRuntimeInterfacePrerequisiteResultV1)
            and result.record.runtime_interface_prerequisite_id
