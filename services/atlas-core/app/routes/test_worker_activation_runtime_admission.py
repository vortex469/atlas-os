"""P3 API locks for v0.54 worker activation runtime admission evidence."""
...
from app.worker_activation_runtime_admission import test_contract as p1
    ...
    *,
    ...
    def read_owned(**kwargs):
        if (
            ...
            or kwargs["prerequisite_id"] != facts.create.prerequisite_id
        ):
            reader.calls += 1
            return None
    ...
    if service_installed:
        application.state.worker_activation_runtime_admission_service = (
            admission_service
        )
    collection = (
        f"/api/v1/installation/candidate-records/{prerequisite.candidate_record_id}"
        "/worker-activation-runtime-admissions"
    )
    return (
...
def test_auth_csrf_success_readback_and_exact_no_replay(tmp_path: Path, facts) -> None:
    ...
    body = made.json()
    assert body["worker_activation_runtime_admission_recorded"] is True
    ...
    fetched = client.get(
        f"{url}/{body['record']['runtime_admission_id']}",
        cookies=_cookies(session),
    )
    ...
    foreign = sessions.create(
        OperatorCredential(
            ...
            password_hash="unused",
            permissions=(INSTALLATION_WORKER_ACTIVATION_RUNTIME_ADMISSION_READ,),
        )
    )
    foreign_get = client.get(
        f"{url}/{body['record']['runtime_admission_id']}", cookies=_cookies(foreign)
    )
...
def test_dedicated_permissions_default_off_and_redaction(tmp_path: Path, facts) -> None:
    ...
    create_client, create_session, _, _create, _, url, _ = _application(
        tmp_path / "create",
        facts,
        permissions=(INSTALLATION_WORKER_ACTIVATION_RUNTIME_ADMISSION_EVALUATE,),
    )
...
def test_permissions_openapi_method_and_authority_isolation() -> None:
    ...
    assert INSTALLATION_WORKER_ACTIVATION_RUNTIME_ADMISSION_EVALUATE == PERMISSION
    assert INSTALLATION_WORKER_ACTIVATION_RUNTIME_ADMISSION_EVALUATE == (
        "installation.execution.worker_activation_runtime_admission.evaluate"
    )
    assert INSTALLATION_WORKER_ACTIVATION_RUNTIME_ADMISSION_READ == (
        "installation.execution.worker_activation_runtime_admission.read"
    )
    assert {
        INSTALLATION_WORKER_ACTIVATION_RUNTIME_ADMISSION_EVALUATE,
        INSTALLATION_WORKER_ACTIVATION_RUNTIME_ADMISSION_READ,
    ...
    item = f"{collection}/{{runtime_admission_id}}"
    ...
    for parameter in paths[item]["get"]["parameters"]:
        examples = {
            ...
            "runtime_admission_id": "a70ea6f4-18ba-57f3-867e-f5eae39bfb2d",
        }
    ...
    assert not any(
        ...
        for path in paths
        if "worker-activation-runtime-admissions" in path
        for segment in path.split("worker-activation-runtime-admissions", 1)[1]
        .strip("/")
        .split("/")
        if segment
    )
...
def test_route_has_no_runtime_effect_imports_or_broad_calls() -> None:
    path = Path(__file__).with_name("worker_activation_runtime_admission.py")
    ...
    allowed_contract_import = "app.worker_activation_runtime_admission.contract"
    assert not [
        name
        for name in imports
...
def test_production_startup_does_not_construct_v054_admission_service() -> None:
    source = Path(__file__).parents[1].joinpath("main.py").read_text(encoding="utf-8")
    assert "worker_activation_runtime_admission_service" not in source
    assert "create_worker_activation_runtime_admission_service" not in source
...
def test_lineage_isolation_staleness_and_permanent_reservation(
    ...
    assert made.status_code == 201
    item_id = made.json()["record"]["runtime_admission_id"]
...
def test_bounded_input_and_redacted_untrusted_service_outputs(
    ...
    assert made.status_code == 201
    item = f"{url}/{made.json()['record']['runtime_admission_id']}"
    real = service.get(
        authenticated_operator_id=made.json()["record"]["operator_id"],
        permission_verified=True,
        runtime_admission_id=made.json()["record"]["runtime_admission_id"],
...
def test_chunked_body_stops_at_bound_without_content_length() -> None:
    ...
    from app.routes.worker_activation_runtime_admission import _body
    from app.worker_activation_runtime_admission.contract import (
        MAX_CREATE_BYTES,
    )
...
def test_reparse_every_service_output_including_constructed_models(
    ...
    result = service.create(
        ...
        permission_verified=True,
        candidate_record_id=facts.candidate_record_id,
        idempotency_key="v054-output-validation",
        correlation_id="test",
    )
    assert isinstance(result, c.WorkerActivationRuntimeAdmissionResultV1)
    item = f"{url}/{result.record.runtime_admission_id}"
    if target == "result":
...
def test_corrupted_nested_prerequisite_cannot_create_evidence(tmp_path, facts):
    ...
    assert response.status_code == 422
    assert counts(
        client._app.state.worker_activation_runtime_admission_service._store
    ) == (0, 0, 0)


def test_exact_registered_surface_and_zero_agent_worker_consumers():
    collection = "/installation/candidate-records/{candidate_record_id}/worker-activation-runtime-admissions"
    assert {
        (route.path, method) for route in router.routes for method in route.methods
    } == {
        (collection, "GET"),
        (collection, "POST"),
        (collection + "/{runtime_admission_id}", "GET"),
    }
    ...
    for directory in ("atlas-agent", "atlas-execution-worker"):
        for path in (root / "services" / directory).rglob("*.py"):
            ...
            assert "worker_activation_runtime_admission" not in source, path
            assert "worker-activation-runtime-admissions" not in source, path
...
def test_valid_service_response_must_match_exact_request(tmp_path, facts, monkeypatch, field):
    ...
    result = service.create(
        ...
        permission_verified=True,
        candidate_record_id=facts.candidate_record_id,
        idempotency_key="v054-response-linkage",
        correlation_id="test",
    )
...
def test_unexpected_dependency_http_errors_are_redacted(tmp_path, facts, monkeypatch):
    ...
    monkeypatch.setattr(
        client._app.state.worker_activation_runtime_admission_service, "create", fail
    )
    response = client.post(
        url,
        json=create.model_dump(mode="json"),
...
def test_ambiguous_headers_fail_before_read_or_reservation(
    ...
    assert reader.calls == 0
    assert counts(client._app.state.worker_activation_runtime_admission_service._store) == (0, 0, 0)
...
def test_api_durable_lineage_restart_expiry_and_corrupt_readback(
    ...
    from app.worker_activation_runtime_admission.test_service_store import counts
    from app.worker_activation_runtime_prerequisite.test_service_store import (
    ...
    service._prerequisite_reader = WorkerActivationRuntimeAdmissionPrerequisiteStoreReader(
        store=type(journal)(journal.database_path), clock=clock,
    )
    payload = c.build_create(
        receipt=record,
        receipt_status=c.v053.derive_status(record, evaluated_at=record.recorded_at),
    ...
    assert made.status_code == 201
    admitted = c.WorkerActivationRuntimeAdmissionResultV1.model_validate_json(made.text)
    assert admitted.record.worker_activation_runtime_prerequisite.model_dump_json() == record.model_dump_json()
    assert journal.database_path.read_bytes() == prior_bytes
    service._store = WorkerActivationRuntimeAdmissionStore(service._store.database_path)
    ...
    item = f"{url}/{admitted.record.runtime_admission_id}"
