"""Guarded Core API for v0.49 controlled worker queue claim admission."""
...
router = APIRouter(
    ...
    tags=["Controlled Worker Queue Claim Admission"],
)

_ERRORS = {
    code: {"model": ControlledWorkerQueueClaimAdmissionResultV1}
    for code in (401, 403, 404, 409, 413, 415, 422, 429, 503)
}


async def _body(request: Request) -> ControlledWorkerQueueClaimAdmissionCreateV1:
...
def _service(request: Request):
    value = getattr(
        request.app.state, "controlled_worker_queue_claim_admission_service", None
    )
    if value is None:
...
def _service_response(result: ControlledWorkerQueueClaimAdmissionResultV1) -> JSONResponse:
    ...
    status_code = {
        ...
        "v048_activation_evidence_not_active": 409,
        "v048_activation_evidence_not_recorded": 409,
...
@router.post(
    ...
    response_model=ControlledWorkerQueueClaimAdmissionResultV1,
    status_code=201,
    responses=_ERRORS,
    summary="Record bounded controlled worker queue claim admission evidence",
    openapi_extra={
        "parameters": [
            {
        ...
        "requestBody": {
            ...
            "content": {
                "application/json": {
                    "schema": (
                        ControlledWorkerQueueClaimAdmissionCreateV1.model_json_schema()
                    )
                }
            },
        },
...
async def create_controlled_worker_queue_claim_admission(
    request: Request,
    response: Response,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> ControlledWorkerQueueClaimAdmissionResultV1 | JSONResponse:
...
@router.get(
    "/{candidate_record_id}/controlled-worker-queue-claim-admissions",
    response_model=ControlledWorkerQueueClaimAdmissionCollectionV1,
    responses=_ERRORS,
    summary="List owned controlled worker queue claim admission evidence",
)
async def list_controlled_worker_queue_claim_admissions(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> ControlledWorkerQueueClaimAdmissionCollectionV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if (
...
@router.get(
    "/{candidate_record_id}/controlled-worker-queue-claim-admissions/{admission_id}",
    response_model=ControlledWorkerQueueClaimAdmissionResultV1,
    responses=_ERRORS,
    summary="Read owned controlled worker queue claim admission evidence",
)
async def get_controlled_worker_queue_claim_admission(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
