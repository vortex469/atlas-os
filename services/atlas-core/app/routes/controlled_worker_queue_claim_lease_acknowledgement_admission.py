"""Guarded Core API for v0.51 queue claim/lease/ack admission evidence."""
...
from app.controlled_worker_queue_claim_lease_acknowledgement_admission.contract import (
    ...
    ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionRedactedErrorV1,
    ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionResultV1,
    opaque_fingerprint,
    parse_create_json,
)
...
from app.operator_auth.models import (
    INSTALLATION_CONTROLLED_WORKER_QUEUE_CLAIM_LEASE_ACK_ADMISSION_READ,
    INSTALLATION_CONTROLLED_WORKER_QUEUE_CLAIM_LEASE_ACK_ADMISSION_RECORD,
)

_UUID4 = r"^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{12}$"
_UUID5 = r"^[0-9a-f]{8}-[0-9a-f]{4}-5[0-9a-f]{3}-[89ab][0-9a-f]{12}$"
...
router = APIRouter(
    ...
    tags=["Controlled Worker Queue Claim Lease Acknowledgement Admission"],
)

_ERRORS = {
    code: {"model": ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionResultV1}
...
def _service(request: Request):
    value = getattr(
        request.app.state,
        "controlled_worker_queue_claim_lease_acknowledgement_admission_service",
        None,
    )
    if value is None:
...
def _correlation(value: str):
    ...
    return opaque_fingerprint(
        "atlas:controlled-worker-queue-claim-lease-acknowledgement-admission-correlation:v1",
        safe,
    )
...
def _error_result(
    ...
    return ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionResultV1(
        ok=False,
        outcome="failure",
        record=None,
        status=None,
        error=ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionRedactedErrorV1(
...
def _service_response(
    result: ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionResultV1,
) -> JSONResponse:
    ...
    status_code = {
        ...
        "v050_prerequisite_not_active": 409,
        "v050_prerequisite_not_frozen": 409,
...
@router.post(
    "/{candidate_record_id}/controlled-worker-queue-claim-lease-acknowledgement-admissions",
    response_model=ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionResultV1,
    status_code=201,
    responses=_ERRORS,
    summary="Record bounded queue claim/lease/ack admission evidence",
    openapi_extra={
        "parameters": [
            {
        ...
        "requestBody": {
            ...
            "content": {
                "application/json": {
                    "schema": (
                        ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionCreateV1.model_json_schema()
                    )
                }
            },
        },
...
async def create_controlled_worker_queue_claim_lease_acknowledgement_admission(
    request: Request,
    response: Response,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionResultV1 | JSONResponse:
...
@router.get(
    "/{candidate_record_id}/controlled-worker-queue-claim-lease-acknowledgement-admissions",
    response_model=ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionCollectionV1,
    responses=_ERRORS,
    summary="List owned queue claim/lease/ack admission evidence",
)
async def list_controlled_worker_queue_claim_lease_acknowledgement_admissions(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
) -> ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionCollectionV1 | JSONResponse:
    correlation_id = request_id_for(request)
    if (
...
@router.get(
    "/{candidate_record_id}/controlled-worker-queue-claim-lease-acknowledgement-admissions/{admission_id}",
    response_model=ControlledWorkerQueueClaimLeaseAcknowledgementAdmissionResultV1,
    responses=_ERRORS,
    summary="Read owned queue claim/lease/ack admission evidence",
)
async def get_controlled_worker_queue_claim_lease_acknowledgement_admission(
    request: Request,
    candidate_record_id: Annotated[
        str, WithJsonSchema({"type": "string", "pattern": _UUID4})
    ],
    ...
    if (
        ...
        and result.record.candidate_record_id != candidate_record_id
    ):
