...
function Stub({ result }: { result: WorkerAdmissionStubResultV1 }) {
    ...
    return <li className="rounded border border-slate-800 p-3 text-sm">
        <h6 className="font-semibold">{status.lifecycle === "active" ? "Active worker-admission-stubbed evidence" : "Expired worker-admission-stubbed evidence"}</h6>
        <p className="mt-1">Eligibility: {stub.eligibility}; lifecycle: {status.lifecycle}; disposition: {result.disposition}; intake state: {intake.intake_state}; protocol: {intake.intake_protocol}.</p>
        <p className="mt-1">Recorded {stub.recorded_at}; valid until {stub.valid_until}; observed {status.observed_at}. Freshness is inherited for at most 30 seconds. Expiry never refreshes, starts a worker, creates a queue, or authorizes execution.</p>
        ...
        <dl className="mt-3 grid gap-2 sm:grid-cols-2">
            ...
            <Value name="Admission intent ID" value={intent.intent_id} /><Value name="Admission intent" value={intent.intent} /><Value name="Intent fingerprint" value={intent.intent_fingerprint.value} /><Value name="Intake fingerprint" value={intake.intake_fingerprint.value} />
        </dl>
        <details className="mt-3" open><summary>Inherited byte-exact sandbox, resource, network, and filesystem ceilings</summary><dl className="mt-2 grid gap-2 sm:grid-cols-2">
        ...
        <details className="mt-3"><summary>Required v0.20–v0.37 linkage and runner binding evidence</summary><dl className="mt-2 grid gap-2 sm:grid-cols-2">
