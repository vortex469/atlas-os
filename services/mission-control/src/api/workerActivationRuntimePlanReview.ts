...
import { parseWorkerActivationRuntimePlan } from "./workerActivationRuntimePlan";
...
const SCHEMA = "worker-activation-runtime-plan-review";
const MARKER = "worker_activation_runtime_plan_review_recorded";
...
function check(value: unknown): asserts value {
    if (!value) throw new Error("Plan review evidence is unavailable.");
}
...
const PROFILE = "core_owned_reference_only_runtime_plan_review_v1";
const FINDINGS = ["exact_plan_lineage", "fixed_design_consistent", "unresolved_interfaces_preserved"];
const COMMON = ["runtime_admission_id", "profile", "findings", "runtime_plan_review_id", "runtime_plan_id", "prerequisite_id", "operator_id", "candidate_record_id", "admission_id", "recorded_at", "valid_until", "lifecycle", "eligibility", "blockers", "runtime_plan_review_record_fingerprint", "worker_activation_runtime_plan_recorded", MARKER];
function record(value: unknown, candidateId: string, operatorId: string) {
    const raw = object(value);
    closed(raw, [...COMMON, "subject_fingerprint", "idempotency_key_fingerprint", "worker_activation_runtime_plan", "worker_activation_runtime_plan_status"]);
    ...
    check(raw.worker_activation_runtime_plan_recorded === true);
    check(raw.profile === PROFILE && sameEvidence(raw.findings, FINDINGS));
    ...
    const fingerprints = { subject: fp(raw.subject_fingerprint), idempotency_key: fp(raw.idempotency_key_fingerprint), runtime_plan_review_record: fp(raw.runtime_plan_review_record_fingerprint) };
    // Adapter envelope for the pure predecessor parser only; never returned as Core evidence.
    const nested = object(raw.worker_activation_runtime_plan);
    const receipt = parseWorkerActivationRuntimePlan({
        ...
        schema: "worker-activation-runtime-plan-result-v1",
        record: nested, status: raw.worker_activation_runtime_plan_status,
        exact_duplicate: false, worker_activation_runtime_admission_recorded: nested.worker_activation_runtime_admission_recorded, worker_activation_runtime_plan_recorded: nested.worker_activation_runtime_plan_recorded,
    }, candidateId, operatorId);
    check(receipt.runtimeAdmissionId === raw.runtime_admission_id);
    check(receipt.runtimePlanId === raw.runtime_plan_id && receipt.prerequisiteId === raw.prerequisite_id && receipt.admissionId === raw.admission_id);
    ...
    return { raw, receipt, runtimeAdmissionId: receipt.runtimeAdmissionId, runtimePlanReviewId: raw.runtime_plan_review_id, runtimePlanId: raw.runtime_plan_id, prerequisiteId: raw.prerequisite_id, admissionId: raw.admission_id, candidateId, operatorId, recordedAt, validUntil, fingerprints };
}
...
export async function getWorkerActivationRuntimePlanReview(prerequisite: WorkerActivationRuntimePlan): Promise<WorkerActivationRuntimePlanReview | null> {
    const { candidateId, operatorId, admissionId, prerequisiteId, runtimePlanId } = prerequisite;
    check(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(candidateId) && UUID5.test(admissionId) && UUID5.test(prerequisiteId) && UUID5.test(runtimePlanId));
    const path = `/installation/candidate-records/${encodeURIComponent(candidateId)}/worker-activation-runtime-plan-reviews`;
    ...
    check(new Set(records.map((item) => item.runtimePlanReviewId)).size === records.length);
    const matches = records.filter((item) => item.runtimePlanId === runtimePlanId);
    ...
    function parentMatches(item: ReturnType<typeof record>) {
        ...
        check(item.runtimePlanId === runtimePlanId && item.runtimeAdmissionId === prerequisite.runtimeAdmissionId);
        check(sameEvidence(item.receipt.exactRecord, prerequisite.exactRecord));
    }
    parentMatches(selected);
    const detail = await atlas.get<unknown>(`${path}/${encodeURIComponent(selected.runtimePlanReviewId)}`, { withCredentials: true });
    const parsed = parseWorkerActivationRuntimePlanReview(detail.data, candidateId, operatorId);
...
function parseWorkerActivationRuntimePlanReview(value: unknown, candidateId: string, operatorId: string): WorkerActivationRuntimePlanReview {
    ...
    check(result.schema === `${SCHEMA}-result-v1` && result[MARKER] === true && result.worker_activation_runtime_plan_recorded === true && typeof result.exact_duplicate === "boolean");
    ...
    for (const key of COMMON) {
        if (key !== "lifecycle" && key !== "runtime_plan_review_record_fingerprint") check(JSON.stringify(status[key]) === JSON.stringify(parsed.raw[key]));
    }
    check(fp(status.runtime_plan_review_record_fingerprint).value === parsed.fingerprints.runtime_plan_review_record.value);
    ...
    return {
        runtimePlanReviewId: parsed.runtimePlanReviewId, profile: PROFILE, findings: [...FINDINGS], runtimeAdmissionId: parsed.runtimeAdmissionId, runtimePlanId: parsed.runtimePlanId, prerequisiteId, admissionId, candidateId, operatorId, exactRecord: parsed.raw,
        ...
        fingerprints: { ...parsed.fingerprints, status: fp(status.status_fingerprint), v055_plan_record: parsed.receipt.fingerprints.runtime_plan_record, v055_plan_status: parsed.receipt.fingerprints.status },
        authority: Object.fromEntries(CLOSED_RUNTIME_AUTHORITY.map((key) => [key, status[key]])) as WorkerActivationRuntimePlanReview["authority"],
        lineage: { record: parsed.raw.worker_activation_runtime_plan, status: parsed.raw.worker_activation_runtime_plan_status },
    };
}
