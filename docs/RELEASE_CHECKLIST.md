...
- [x] Retain the v0.57 pair, complete lineage, exact limits, inventory and
  authority ceiling with integrated v0.61 hardening.
- [x] Freeze retention and defer successor P1-P5; reserve no new consumer,
  schema, marker, route or runtime authority.
- [x] Synchronize normative architecture, P0 supersession notice, roadmap and
  Sync evidence.
- [ ] Resolve repository-integrated v0.61 and inherited predecessor acceptance.
- [ ] Complete the independent full Core Ruff/validation and unrestricted
  worker API obligations recorded by v0.61 P5.

[Normative v0.62 boundary](architecture/v0.62-authority-boundary.md) and
[Sync validation](architecture/v0.62-sync-validation.md) record planning
completion, not release acceptance. No tag, publication or runtime enablement
is authorized.

## Atlas v0.61 P5 — retained release gates (2026-09-14)

- [x] Verify exact integrated v0.60/v0.61 ancestors at `5ec04904`.
- [x] Reconcile Sync and retained P1-P4 in the normative [v0.61 boundary](architecture/v0.61-authority-boundary.md).
- [x] Add v0.61 successor-marker refusal across eight production areas and shared
  Core/UI evidence envelopes, including false-valued claims, without changing
...
- [x] Run retained v0.61 P1/P2 contract/durable regressions and historical
  closure; exact guarded API isolation is recorded in
  [v0.61 P3 evidence](architecture/v0.61-p3-validation.md).
- [x] Current full Mission Control test/build/lint: 174 files, 2,319 tests, exit 0;
  build and lint pass with the existing recorded warnings.
- [ ] Repository-integrated predecessor release acceptance, including v0.60.
...
- [ ] Unrestricted worker API validation: threaded API invocation limitation.
- [ ] Release acceptance after all required gates pass; no tag/publication implied.

[P5 evidence](architecture/v0.61-p5-validation.md) records exact commits,
commands, current results and limits. This synchronization records retained
isolation and open release gates; it is not release acceptance.
...
- [x] Freeze exactly the retained v0.57 pair, IDs, fingerprints, full lineage,
  owner-scoped Core/API semantics, seven blockers and fixed-false authority.
- [x] Defer runtime definition P1-P5: provenance, distinct predicate/consumer
  and expanded-size/time feasibility remain unproved; no successor is selected.
- [x] Synchronize normative architecture, P0 notice and shared roadmap evidence.
- [ ] Close predecessor v0.58/v0.59 required full Mission Control release gates.
- [ ] Before any successor implementation, separately satisfy and freeze the
  reopening requirements; release closure alone is insufficient.

The [normative boundary](architecture/v0.60-runtime-definition-boundary.md) and
[Sync evidence](architecture/v0.60-sync-validation.md) document planning scope.
Historical dependency failures remain historical observations, not a fresh
runtime diagnosis. No production, dependency, deployment or tag change is made.
...
- [x] Inspect the completed repository-supported v0.49 Controlled Worker Queue
  Claim Admission contract, implementation, API/UI structural tests, and
  release checklist before defining v0.50.
- [x] Freeze the remaining missing prerequisite boundary for controlled queue
  claim, lease, and acknowledgement before any worker-start admission,
  controlled worker invocation, worker start, execution-start admission, or
  execution boundary may be considered.
- [x] Preserve exact v0.49 lineage, ownership, freshness/expiry,
  fingerprints, inherited limits, permanent idempotency and subject no-replay,
  redaction, ambiguity handling, default-off construction, API/UI isolation,
  Home Assistant blocking, Agent/execution-worker zero-consumer behavior, and
  absence of `compose.execution-smoke.override.yaml`.
...
documentation-only [v1 contract](architecture/one-shot-controlled-dequeue-boundary-v1.md).

- [x] Inspect the repository-supported v0.44 Controlled Dequeue Admission,
  v0.43 Queue Observation and Enqueue Receipt, and v0.42 One-Shot Live
  Enqueue contracts, implementation, API/UI structural tests, and release
  checklist before defining v0.45.
- [x] Freeze exactly one new future authority: at most one explicitly
  authorized, single-use controlled dequeue of the exact admitted inert v0.42
  queue item after one active valid same-owner v0.44 controlled dequeue
  admission record.
- [x] Freeze exact prerequisite lineage, queue identity, queue-item identity,
  ...
  goldens, and must-not-change contracts.
- [x] Freeze dedicated scope `installation_one_shot_controlled_dequeue_only`
  and dedicated
  `installation.execution.one_shot_controlled_dequeue.record` and `.read`
  permissions for later phases.
- [x] Freeze one-shot controlled dequeue as exact-item receipt evidence only:
  it does not authorize or cause autonomous queue polling, work discovery,
  claim, lease, acknowledgement, queue item mutation or replacement, worker
  start or invocation, Agent invocation, execution authorization/start,
...
- [ ] P2 - explicitly constructed append-only Core evidence service/store with
  injected owner-scoped v0.44/v0.43/v0.42 readers, one injected single-use
  dequeue adapter, permanent no-replay, and indeterminate append/dequeue
  handling.
